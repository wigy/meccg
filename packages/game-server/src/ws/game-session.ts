...
import { loadCardPool, createRng, buildMovementMap, createGame, reduce, startCapture, flushCapture, Phase, computeTournamentBreakdown, computeLegalActions, withMetaActions, stampActionIds, canonicalActionKey, extractActionCardDefs, redactActionForAudience, validateDeck, Alignment, CHARACTER_CARD_TYPES } from '@meccg/shared';
...
export class GameSession {
  ...
  private broadcastState(lastAction?: GameAction): void {
    ...
    for (const [, { ws, playerId }] of this.players.entries()) {
      ...
      // Concede and the early Free Council handshake are player-facing
      // meta-actions layered on here, per seat, rather than inside the
