 ...
 * last credit addition, rounded up to the nearest hundred.
 */
...
describe('pendingTopUp', () => {
  ...
  test('rounds a partial hundred up', () => {
    expect(pendingTopUp([entry(-5)])).toBe(100);
    expect(pendingTopUp([entry(-117)])).toBe(200);
  });

  test('leaves an exact hundred alone', () => {
    expect(pendingTopUp([entry(-100)])).toBe(100);
    expect(pendingTopUp([entry(-60), entry(-140)])).toBe(200);
  });
});
