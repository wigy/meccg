...
import { ARAGORN, RIVENDELL } from '../../../index.js';
import type { CardDefinitionId, CardInstance, ManifestId, SiteCard } from '../../../index.js';
import {
...
describe('METD §4 — isManifestationDefeated', () => {
  ...
  test('returns true once any chain card lands in either eliminated pile', () => {
    ...
    const state = addToPile(buildSimpleTwoPlayerState(), HAZARD_PLAYER, 'outOfPlayPile', ahuntInst);
...
describe('METD §4 — Lair auto-attack suppression', () => {
  ...
  test('lair loses its Dragon auto-attack once the Dragon is defeated', () => {
    ...
    const state = addToPile(buildSimpleTwoPlayerState(), RESOURCE_PLAYER, 'outOfPlayPile', basicInst);
    const lonely = state.cardPool[LONELY_MOUNTAIN] as SiteCard;
