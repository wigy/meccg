 ...
 * @module td-67.test
 *
 * Card test: Searching Eye (td-67)
 * Type: hazard-event (short)
 *
 * td-67 is the TD reprint of le-136 (identical text). It carries two effects:
 *   1. on-event self-enters-play → cancel-chain-entry on a target whose source
 *      card has an effect with `requiredSkill: "scout"` (cancel a scout-skill
 *      card on the chain before it resolves, or cancel the ongoing effect of a
 *      card that required scout to play).
 *   2. on-guard-reveal (trigger resource-short-event) → the same
 *      cancel-chain-entry, letting it be revealed during the opponent's site
 *      phase to cancel and discard a scout-skill card before it resolves.
 *
 * Text:
...
const SEARCHING_EYE = 'td-67' as CardDefinitionId;
...
describe('Searching Eye (td-67) — chain cancel', () => {
...
describe('Searching Eye (td-67) — on-guard reveal (site phase)', () => {
  // PLAYER_1 at Moria with Stealth in hand; Searching Eye placed on-guard on
  // PLAYER_1's company. Rebuilt in beforeEach so mint counters stay
  ...
  test('playing a short without requiredSkill does NOT enqueue an on-guard-window', () => {
    ...
    // hand that declares `requiredSkill`, nothing is intercepted.
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Site,
      recompute: true,
