 ...
 * @module td-1.test
 *
 * Card test: Agburanar Ahunt (td-1)
 ...
 *  hazard creature attack) — 3 strikes at 13/8.
...
describe('Agburanar Ahunt (td-1)', () => {
  beforeEach(() => resetMint());

  test('company moving through Withered Heath triggers ahunt combat with 3 strikes at 13/8', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: AGBURANAR_AHUNT, ...PATH_WITHERED_HEATH });
  ...
  test('company moving through Northern Rhovanion triggers ahunt combat', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: AGBURANAR_AHUNT, ...PATH_NORTHERN_RHOVANION });
  ...
  test('company moving through Iron Hills triggers ahunt combat', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: AGBURANAR_AHUNT, ...PATH_IRON_HILLS });
  ...
  test('company moving through Grey Mountain Narrows triggers ahunt combat', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: AGBURANAR_AHUNT, ...PATH_GREY_MOUNTAIN_NARROWS });
  ...
  test('company moving through non-matching region does not trigger ahunt', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: AGBURANAR_AHUNT, ...PATH_NON_MATCHING });
  ...
  test('non-moving company (empty path) does not trigger ahunt', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: AGBURANAR_AHUNT, ...PATH_EMPTY });
  ...
  test('Southern Rhovanion does NOT trigger without Doors of Night', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: AGBURANAR_AHUNT, ...PATH_SOUTHERN_RHOVANION });
  ...
  test('Woodland Realm does NOT trigger without Doors of Night', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: AGBURANAR_AHUNT, ...PATH_WOODLAND_REALM });
  ...
  test('ahunt long-event stays in cardsInPlay after combat (not moved to kill/discard)', () => {
    ...
    expect(current.players[0].killPile.some(c => c.definitionId === AGBURANAR_AHUNT)).toBe(false);
    expect(current.players[1].discardPile.some(c => c.definitionId === AGBURANAR_AHUNT)).toBe(false);
  });
});
