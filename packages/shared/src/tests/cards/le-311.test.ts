 ...
 * @module le-311.test
 *
 * Card test: Gleaming Gold Ring (le-311)
 * Type: minion-resource-item (subtype: gold-ring, alignment: ringwraith)
 * Corruption: 4, Marshalling Points: 2
 *
 * "May only be played at a Border-hold [{B}] where gold rings are playable.
 *  Bearer must make a corruption check at the end of each of his untap phases.
 *  Discard this ring when tested. If tested, obtain a random value to determine
 *  which ring card may be immediately played: a Spirit Ring (10,11,12+);
 *  a Dwarven Ring (9,10,11,12+); a Magic Ring (1,2,3,4,5,6);
 *  a Lesser Ring (any result). You may search your play deck and discard pile
 *  for a Lesser Ring to be played."
 *
 * Engine support:
 * | # | Rule                                                    | Status      | Notes                                              |
 * |---|---------------------------------------------------------|-------------|---------------------------------------------------|
 * | 1 | +4 corruption points on bearer                          | IMPLEMENTED | itemDef.corruptionPoints summed                   |
 * | 2 | May only be played at Border-hold with gold rings       | IMPLEMENTED | item-play-site filter: siteType+playableResources |
 ...
 * | 6 | Spirit Ring eligible on 10+                             | IMPLEMENTED | ring-test-table row min:10                        |
 * | 7 | Dwarven Ring eligible on 9+                             | IMPLEMENTED | ring-test-table row min:9                         |
 * | 8 | Magic Ring eligible on 1–6                              | IMPLEMENTED | ring-test-table row min:1 max:6                   |
 * | 9 | Lesser Ring always eligible                             | IMPLEMENTED | ring-test-table row min:null max:null             |
 * |10 | Search play deck and discard pile for Lesser Ring       | IMPLEMENTED | ring-test-search, ringPlayOfferActions            |
 ...
 *
 * Site fixtures:
 *   - DALE (le-363):            border-hold, gold-ring playable  ← valid play site
 *   - MORIA (le-392):           shadow-hold, gold-ring playable  ← invalid (not border-hold)
 *   - EASTERLING_CAMP (le-371): border-hold, nothing playable    ← invalid (no gold-ring)
 *   - DOL_GULDUR (le-367):      haven, auto-test-gold-ring       ← for corruption/storage tests
...
describe('Gleaming Gold Ring (le-311)', () => {
  ...
  test('offered at Dale (border-hold with gold-ring playable)', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      recompute: true,
      players: [
        { id: PLAYER_1, companies: [{ site: DALE, characters: [GORBAG] }], hand: [], siteDeck: [ETTENMOORS] },
        { id: PLAYER_2, companies: [{ site: DOL_GULDUR, characters: [SHAGRAT] }], hand: [], siteDeck: [MINAS_MORGUL] },
      ],
      phase: Phase.Site,
    });
    const withRing = addCardToHand({ ...base, phaseState: PLAY_RESOURCES_STATE }, RESOURCE_PLAYER, GLEAMING_GOLD_RING);

    const plays = viableActions(withRing, PLAYER_1, 'play-hero-resource');
    expect(plays.length).toBeGreaterThanOrEqual(1);
  });

  test('not offered at Moria (shadow-hold with gold-ring but not border-hold)', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      recompute: true,
      players: [
  ...
  test('not offered at Easterling Camp (border-hold without gold-ring)', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      recompute: true,
      players: [
        { id: PLAYER_1, companies: [{ site: EASTERLING_CAMP, characters: [GORBAG] }], hand: [], siteDeck: [ETTENMOORS] },
        { id: PLAYER_2, companies: [{ site: DOL_GULDUR, characters: [SHAGRAT] }], hand: [], siteDeck: [MINAS_MORGUL] },
      ],
      phase: Phase.Site,
    });
  ...
  test('untap → org transition enqueues a corruption check for the bearer', () => {
    ...
    const withRing = attachItemToChar(base, RESOURCE_PLAYER, GORBAG, GLEAMING_GOLD_RING);
    ...
    expect(pending[0].kind.reason).toBe('Gleaming Gold Ring');

    const gorbagId = charIdAt(afterPass, RESOURCE_PLAYER);
    expect(pending[0].kind.characterId).toBe(gorbagId);
  });

  test('corruption check fires at a haven too (untap-phase-end fires regardless of site type)', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Untap,
      recompute: true,
  ...
  test('ring-test-table: lesser-ring always eligible (roll 7 — above magic max, below dwarven min)', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Organization,
      recompute: true,
    ...
    // Roll 7 is above magic-ring max (6) and below dwarven-ring min (9) — only lesser-ring
  ...
  test('ring-test-table: magic-ring eligible on roll 1–6', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Organization,
      recompute: true,
  ...
  test('ring-test-table: magic-ring NOT eligible on roll 8', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Organization,
      recompute: true,
  ...
  test('ring-test-table: dwarven-ring eligible on roll 9+', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Organization,
      recompute: true,
  ...
  test('ring-test-table: spirit-ring eligible on roll 10+', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Organization,
      recompute: true,
    ...
    const afterRoll = dispatch({ ...withTest, cheatRollTotal: 12 }, rollActions[0].action);
    const pending = afterRoll.pendingResolutions.filter(r => r.actor === PLAYER_1);
    if (pending[0].kind.type !== 'ring-play-offer') return;
    expect(pending[0].kind.eligibleCategories).toContain('lesser-ring');
    expect(pending[0].kind.eligibleCategories).toContain('dwarven-ring');
  ...
  test('ring is discarded from character when tested', () => {
    ...
    expectInDiscardPile(afterRoll, RESOURCE_PLAYER, GLEAMING_GOLD_RING);
  });
  ...
  // ── Effect 10+11: lesser-ring in play deck is offered as play-ring-after-test ─

  test('lesser-ring in play deck is offered after the test (source: play-deck)', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Organization,
      recompute: true,
  ...
  // ── Effect 12: lesser-ring in discard pile is offered ────────────────────

  test('lesser-ring in discard pile is offered after the test (source: discard-pile)', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Organization,
      recompute: true,
  ...
  test('playing lesser-ring from discard pile removes it from discard and attaches to bearer', () => {
    ...
    // Minor Ring removed from discard pile
    ...
    // Minor Ring now attached to Gorbag
    expect(getCharacter(afterPlay, RESOURCE_PLAYER, GORBAG).items).toHaveLength(1);
  });

  test('player can pass the ring-play-offer without playing a ring from search', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Organization,
      recompute: true,
  ...
  test('ring-test-search does not offer non-lesser-ring cards from deck/discard', () => {
    // The ring-test-search effect has category:"lesser-ring". Cards in the deck
    // that don't carry the "lesser-ring" keyword must NOT be offered via search.
    // We verify by checking no deck/discard actions appear when only the gleaming
    // gold ring (not a lesser-ring) is in the deck.
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Organization,
      recompute: true,
    ...
    const withRing = attachItemToChar(base, RESOURCE_PLAYER, GORBAG, GLEAMING_GOLD_RING);
    // Put a non-lesser-ring gold ring in the deck
    const withGoldRingInDeck = addCardToPlayDeck(withRing, RESOURCE_PLAYER, GLEAMING_GOLD_RING);
    ...
    const playActions = viableActions(afterRoll, PLAYER_1, 'play-ring-after-test');
    // No deck/discard offers — the gold ring in deck is not a "special" ring (subtype !== 'special')
