 ...
 * @module le-383.test
 *
 * Card test: Iron Hill Dwarf-hold (le-383)
 * Type: minion-site (free-hold) · alignment: ringwraith · region: Iron Hills
 *
 * Card text:
 ...
 *
 * A plain minion Free-hold with no special printed rules — everything the card
 * does is structural engine support (site phase flow, item playability gated by
 * `playableResources`, a single static automatic-attack, and haven-path
 * movement from its nearest Darkhaven). No `effects` array is required.
 *
 ...
 * | 2 | sitePath          | OK     | [dark, wilderness, wilderness, wilderness] ({d}{w}{w}{w}) |
 ...
 * | 4 | Haven-path movement (Dol Guldur ↔ site)     | IMPLEMENTED | movement-map.ts                                |
 *
 * Playable: YES
...
describe('Iron Hill Dwarf-hold (le-383)', () => {
  beforeEach(() => resetMint());

  // ─── Automatic attack: Dwarves — 4 strikes with 10 prowess ──────────────────

  test('automatic attack is Dwarves — 4 strikes with 10 prowess', () => {
    const state = buildMinionSitePhaseState({ site: IRON_HILL_DWARF_HOLD, characters: [GORBAG] });
  ...
  test('minor item (Strange Rations) is playable', () => {
    const state = buildMinionSitePhaseState({ site: IRON_HILL_DWARF_HOLD, characters: [GORBAG], hand: [STRANGE_RATIONS] });
  ...
  test('major item (Sable Shield) is playable', () => {
    const state = buildMinionSitePhaseState({ site: IRON_HILL_DWARF_HOLD, characters: [GORBAG], hand: [SABLE_SHIELD] });
  ...
  test('greater item (Scroll of Isildur) is playable — the differentiator from most Free-holds', () => {
    const state = buildMinionSitePhaseState({ site: IRON_HILL_DWARF_HOLD, characters: [GORBAG], hand: [SCROLL_OF_ISILDUR] });
  ...
  test('gold-ring item (Bright Gold Ring — Free-hold where gold rings are playable) is playable', () => {
    const state = buildMinionSitePhaseState({ site: IRON_HILL_DWARF_HOLD, characters: [GORBAG], hand: [BRIGHT_GOLD_RING] });
  ...
  test('greater and gold-ring items are NOT playable at a site lacking those subtypes (Shrel-Kain)', () => {
    ...
    // items that succeed at Iron Hill Dwarf-hold are rejected here, proving the
    // playability comes from le-383's playableResources, not the item itself.
  ...
  // ─── Haven-path movement: Dol Guldur ↔ Iron Hill Dwarf-hold ─────────────────

  test('starter movement from Dol Guldur reaches Iron Hill Dwarf-hold', () => {
    ...
    const starter = reachable.find(
      r => r.movementType === 'starter' && r.site.id === (IRON_HILL_DWARF_HOLD as string),
    );
    expect(starter).toBeDefined();
  });

  test('starter movement from Iron Hill Dwarf-hold reaches Dol Guldur', () => {
    const ironHill = pool[IRON_HILL_DWARF_HOLD as string] as SiteCard;
    const allSites = Object.values(pool).filter(isSiteCard);
    const movementMap = buildMovementMap(pool);

    const reachable = getReachableSites(movementMap, ironHill, allSites);
    const starter = reachable.find(
