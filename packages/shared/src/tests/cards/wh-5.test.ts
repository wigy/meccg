 ...
 * @module wh-5.test
 *
 * Card test: Ill-favoured Fellow (wh-5)
 ...
 *
 * Rules exercised:
 * 1. A company whose only character is the (Half-orc) Ill-favoured Fellow is
 *    COVERT — Not Slay Needlessly can cancel an attack against it. Control: a
 *    plain Orc (Gorbag) in the same slot makes the company overt, so NSN only
 *    gives -2 prowess and cannot cancel.
 ...
 *
 * Fixtures:
 *   ILL_FAVOURED_FELLOW (wh-5)  — minion Half-orc warrior/ranger, body 9, discardBodyCheck [9]
...
const ILL_FAVOURED_FELLOW = 'wh-5' as CardDefinitionId;
...
describe('Ill-favoured Fellow (wh-5)', () => {
  ...
  test('Half-orc company stays covert — Not Slay Needlessly can cancel the attack', () => {
    const base = buildTestState({
      ...
      recompute: true,
      players: [
        { id: PLAYER_1, alignment: Alignment.Ringwraith, companies: [{ site: CARN_DUM, characters: [ILL_FAVOURED_FELLOW] }], hand: [NSN], siteDeck: [MINAS_MORGUL] },
  ...
  test('Half-orc that defeats a creature is NOT offered it as a trophy — combat finalizes', () => {
    const base = buildTestState({
      ...
      recompute: true,
      players: [
        { id: PLAYER_1, alignment: Alignment.Ringwraith, companies: [{ site: CARN_DUM, characters: [ILL_FAVOURED_FELLOW] }], hand: [], siteDeck: [MINAS_MORGUL] },
        { id: PLAYER_2, alignment: Alignment.Wizard, companies: [{ site: DOL_GULDUR, characters: ['tw-168' as CardDefinitionId] }], hand: [], siteDeck: [MINAS_MORGUL] },
      ],
    });

    const halfOrcId = findCharInstanceId(base, RESOURCE_PLAYER, ILL_FAVOURED_FELLOW);
  ...
  test('Body check roll of exactly 9 discards the Half-orc (not eliminated)', () => {
    ...
    const fellowId = findCharInstanceId(state, RESOURCE_PLAYER, ILL_FAVOURED_FELLOW);
    const companyId = companyIdAt(state, RESOURCE_PLAYER);
    const woundedState = setCharStatus(state, RESOURCE_PLAYER, ILL_FAVOURED_FELLOW, CardStatus.Inverted);
