 ...
 * filter — NONE of The Under-courts' attacks are detainment: the 1st Trolls
 ...
 * creatures played normally against a company here alike. Mirror of its
 * sibling The Under-galleries (ba-99); the only difference is Information is
 * NOT playable here (Items minor/major only) and the Trolls attack is 3x10.
 *
 * Data encoding:
 *   - `playableResources: [minor, major]` — filled this pass (was `[]` in the
 *     imported data despite the printed "Playable" line, the recurring BA/LE-
 *     site empty-playableResources bug).
 ...
 * | 3 | Automatic attacks (1st, static Trolls)         | IMPLEMENTED     | Trolls 3x10 in automaticAttacks                           |
...
describe('The Under-courts (ba-98)', () => {
  ...
  test('Balrog company at The Under-courts faces the 1st Troll attack normally, not detainment (integration)', () => {
  ...
  test('minion company here facing Orc-patrol (Shadow-hold keyed): combat.detainment is false (integration)', () => {
    // PLAYER_1 is the Balrog/active player at The Under-courts. PLAYER_2
      ...
      players: [
        {
          ...
          companies: [{ site: THE_UNDER_COURTS, characters: [CROOK_LEGGED_ORC] }],
          hand: [],
    ...
    const ready: GameState = {
      ...
      phaseState: makeMHState({
        ...
        destinationSiteName: 'The Under-courts',
      }),
    };
  ...
  test('baseline: same Orc-patrol vs Balrog company at a shadow-hold WITHOUT the override IS detainment', () => {
    // Swap The Under-courts for hero Moria (tw-413) — a shadow-hold with no
