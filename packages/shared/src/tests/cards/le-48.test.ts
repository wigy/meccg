 ...
 *
 * Characters used:
 *   - le-48  Ufthak (orc, body 8, discardBodyCheck [8]) — subject under test
...
const UFTHAK = 'le-48' as CardDefinitionId;
const ASTERNAK = 'le-1' as CardDefinitionId;   // man, body 7 — hazard player placeholder

const MINAS_MORGUL = 'le-390' as CardDefinitionId; // minion haven (Ufthak home)
...
describe('Ufthak (le-48)', () => {
  ...
  test('discarded to discard pile when mass body check fails', () => {
    ...
    const state = buildTestState({
      ...
      players: [
        { id: PLAYER_1, companies: [{ site: MINAS_MORGUL, characters: [UFTHAK] }], hand: [], siteDeck: [EDORAS] },
    ...
    const ufthakId = findCharInstanceId(s, RESOURCE_PLAYER, UFTHAK);
    ...
    expect(rollAction.characterId).toBe(ufthakId);

    s = dispatch(s, rollAction);

    expectCharNotInPlay(s, RESOURCE_PLAYER, ufthakId);
    const discardDefIds = s.players[0].discardPile.map(c => c.definitionId);
    expect(discardDefIds).toContain(UFTHAK);
    expect(s.players[0].outOfPlayPile.map(c => c.definitionId)).not.toContain(UFTHAK);
  });
  ...
  test('stays in play when mass body check passes', () => {
    ...
    expectCharInPlay(s, RESOURCE_PLAYER, ufthakId);
  });
});
