 ...
 * @module le-391.test
 *
 * Card test: Minas Tirith (le-391)
 * Type: minion-site (free-hold) in Anórien
 ...
 *    Automatic-attacks (2):
 *      (1st) Men — each character faces 1 strike with 9 prowess
 ...
 *
 * Data encoding:
 *   - Men attack: `combatRules: ["each-character"]`, prowess 9, no `appliesTo`
 *     (faced by all companies).
 *   - Dúnedain attack: 4 strikes, prowess 10, `appliesTo: "overt"`.
 ...
 * | 4 | region            | OK     | "Anórien" — valid region                               |
 * | 5 | playableResources | OK     | [minor, major, gold-ring, information] — matches text  |
 * | 6 | automaticAttacks  | OK     | Men (each-character, p9) + Dúnedain (4×p10, overt-only)|
 ...
 * | 2 | Haven path movement              | IMPLEMENTED | Minas Morgul ↔ Minas Tirith via starter movement     |
 * | 3 | Auto-attack (each-character)     | IMPLEMENTED | strikesTotal = company size, 1 strike per character  |
 * | 4 | Auto-attack (multi-strike)       | IMPLEMENTED | Dúnedain: 4 strikes total, prowess 10                |
...
const MINAS_TIRITH_LE = 'le-391' as CardDefinitionId;
const MINAS_MORGUL = 'le-390' as CardDefinitionId;
const MINAS_TIRITH_TW = 'tw-412' as CardDefinitionId;
    ...
    players: [
      {
        ...
        companies: [{ site: MINAS_TIRITH_LE, characters }],
        hand: [],
...
describe('Minas Tirith (le-391)', () => {
  beforeEach(() => resetMint());

  // ─── Movement: Minas Morgul ↔ Minas Tirith ─────────────────────────────────

  test('starter movement from Minas Morgul reaches Minas Tirith (le-391)', () => {
    ...
    const starter = reachable.find(
      r => r.movementType === 'starter' && r.site.id === (MINAS_TIRITH_LE as string),
    );

    expect(starter).toBeDefined();
  });

  test('starter movement from Rivendell does NOT reach minion Minas Tirith (le-391)', () => {
  ...
  // Minas Tirith is a free-hold, so the §3.II.2.R1 dark-hold/shadow-hold
  // branch does NOT fire. Detainment comes solely from the site's
  ...
  test('covert Ringwraith company: Men auto-attack is detainment (site effect)', () => {
    const siteDef = pool[MINAS_TIRITH_LE as string] as SiteCard;
  ...
  test('covert company: Men attack is each-character detainment with prowess 9', () => {
  ...
  test('overt company: after Men, faces the Dúnedain attack (4 strikes, prowess 10, not detainment)', () => {
    const state = siteAutoAttackStep([ORC_CAPTAIN, THE_MOUTH]);
    ...
    expect(afterDun.combat!.strikesTotal).toBe(4);
    // 4 strikes total (not each-character).
    expect(afterDun.combat!.eachCharacterFacesOneStrike).toBeUndefined();
    expect(afterDun.combat!.detainment).toBe(false);
  });

  // ─── The TW (hero) and LE (minion) Minas Tirith are distinct sites ─────────

  test('starter movement from Minas Morgul does NOT reach the hero Minas Tirith (tw-412)', () => {
    ...
    const reachesHero = reachable.find(
      r => r.movementType === 'starter' && r.site.id === (MINAS_TIRITH_TW as string),
    );
