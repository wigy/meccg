 ...
 * @module le-404.test
 *
 * Card test: Southron Oasis (le-404)
 ...
 * rulings, same shape as Easterling Camp le-371 and Minas Tirith le-391's first
 * attack): the single Men automatic-attack is faced by EVERY company entering
 * the site — it is a detainment attack against a covert company and a regular
 * (wounding) attack against an overt company. Southron Oasis is a border-hold,
 * so the §3.II.2.R1 dark-hold/shadow-hold auto-detainment branch does NOT fire
 * and a Man keyed to a border-hold does not satisfy the §3.II.2.R2 Shadow-land
 ...
 * | 4 | region            | OK     | "Harondor" — valid region                      |
 ...
 * | 2 | Starter movement from haven      | IMPLEMENTED | Minas Morgul → Southron Oasis (nearestHaven)         |
...
const SOUTHRON_OASIS = 'le-404' as CardDefinitionId;
    ...
    players: [
      {
        ...
        companies: [{ site: SOUTHRON_OASIS, characters }],
        hand: [],
...
describe('Southron Oasis (le-404)', () => {
  beforeEach(() => resetMint());

  // ─── Detainment helper: covert vs overt at a border-hold ────────────────────
  // Southron Oasis is a border-hold, so the §3.II.2.R1 dark-hold/shadow-hold
  ...
  test('covert Ringwraith company: Men auto-attack is detainment (site effect)', () => {
    const siteDef = pool[SOUTHRON_OASIS as string] as SiteCard;
