 ...
 * @module le-102.test
 *
 * Card test: Arouse Denizens (le-102)
 ...
 *       This is an LE reprint of Arouse Denizens (tw-6): identical text and
 *       identical effects.
 *   - duplication-limit scope:site max:1 ("Cannot be duplicated on a given site")
 *
 * Engine support: identical machinery to Arouse Denizens (tw-6) and Arouse
 * Defenders (le-101), generalized over `siteTypes` and `uncancelable`.
 * | # | Rule                                                           | Status |
 * |---|----------------------------------------------------------------|--------|
 ...
 *
 * Playable: YES. Certified: 2026-09-24.
 */
...
const AROUSE_DENIZENS = 'le-102' as CardDefinitionId;
...
function withBoost(state: GameState, siteDefId: CardDefinitionId): GameState {
  ...
  return addConstraint(state, {
    source: 'le102-src' as CardInstanceId,
...
describe('Arouse Denizens (le-102)', () => {
