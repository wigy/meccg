 ...
 *   race elf, keywords ["agent"], prowess 3, body 8, mind 4, directInfluence 1,
 *   marshallingPoints 1, skills scout/ranger, homesite "Thranduil's Halls,
 *   Sarn Goriwing". Unique.
 *
 * Nimloth's effects and text are identical to Elwen (dm-8), already certified
 * — see dm-8.test.ts for the sibling card's rationale. This file re-exercises
 * the same three rules with Nimloth as the acting agent, using DI values that
 * fit her lower base direct influence (1, vs Elwen's 2).
 *
 ...
 *
 * Rules exercised:
 * 1. +2 DI vs Elves (follower control): Nimloth (base DI 1) can take an elf
 *    (Haldir, mind 3) as a follower — avail DI 1+2=3 >= 3 — but cannot take a
 *    non-elf of the same mind bracket (Orc Chieftain, orc, mind 4) — avail DI
 *    stays 1.
 * 2. The bonus is conditional: it does not inflate Nimloth's effective DI stat.
 ...
 *    faction (Wood-elves); no bonus against a non-elf faction (Snaga-hai).
 * 5. "Agent only: may move to a Haven": Nimloth-as-agent at Amon Hen (nearest
 ...
 *
 * Fixtures:
 *   NIMLOTH (dm-20)       — minion elf agent, DI 1, +2 vs elves/elf factions
 *   ELWEN (dm-8)          — minion elf, mind 5 (opponent elf char target)
 *   HALDIR (tw-164)       — elf, mind 3 (elf follower target)
 ...
 *   THRANDUILS_HALLS (le-408) — minion free-hold (Wood-elves' playable site)
...
const NIMLOTH = 'dm-20' as CardDefinitionId;
const ELWEN = 'dm-8' as CardDefinitionId;          // minion elf, mind 5
const HALDIR = 'tw-164' as CardDefinitionId;       // elf, mind 3
...
describe('Nimloth (dm-20)', () => {
  beforeEach(() => resetMint());

  // ── Rule: "+2 direct influence against Elves" — follower-control DI ─────────

  test('+2 DI vs Elves raises Nimloth\'s available DI when controlling an elf (Haldir)', () => {
    // The follower-control influence check (reason: influence-check) resolves
    // Nimloth's conditional DI against the target's race. Against an elf
    // target (Haldir, mind 3) the bonus fires: base DI 1 + 2 = 3 (>= Haldir's
    // mind 3 → she could take him as a follower). With no specific target it
    // does not fire.
    const state = buildTestState({
      ...
      recompute: true,
      players: [
        { id: PLAYER_1, alignment: Alignment.Ringwraith, companies: [{ site: MINAS_MORGUL, characters: [NIMLOTH] }], hand: [], siteDeck: [DOL_GULDUR] },
        { id: PLAYER_2, companies: [{ site: DOL_GULDUR, characters: [CIRYAHER] }], hand: [], siteDeck: [MORIA] },
      ],
    });

    const nimlothId = findCharInstanceId(state, RESOURCE_PLAYER, NIMLOTH);
    const haldirDef = pool[HALDIR as string] as CharacterCard;
    expect(availableDI(state, nimlothId, state.players[RESOURCE_PLAYER], haldirDef)).toBe(3);
    expect(availableDI(state, nimlothId, state.players[RESOURCE_PLAYER])).toBe(1);
  });

  test('+2 DI bonus does NOT apply when controlling a non-elf (Orc Chieftain)', () => {
    // Orc Chieftain is an orc (NOT an elf). Nimloth's bonus is race-gated, so
    // her available DI against him stays at the base 1 (< his mind 4).
    const state = buildTestState({
      phase: Phase.Organization,
      activePlayer: PLAYER_1,
      recompute: true,
    ...
    expect(availableDI(state, nimlothId, state.players[RESOURCE_PLAYER], orcDef)).toBe(1);
  });

  test('the +2 bonus is conditional — it does not inflate Nimloth\'s effective DI stat', () => {
    const state = buildTestState({
      phase: Phase.Organization,
      activePlayer: PLAYER_1,
      recompute: true,
    ...
    expect(getCharacter(state, RESOURCE_PLAYER, NIMLOTH).effectiveStats.directInfluence).toBe(1);
  });
  ...
  test('+2 DI vs Elves applies to an opponent-influence attempt against an elf character', () => {
    const base = buildTestState({
      ...
      recompute: true,
      players: [
        { id: PLAYER_1, alignment: Alignment.Ringwraith, companies: [{ site: MORIA, characters: [NIMLOTH] }], hand: [], siteDeck: [DOL_GULDUR] },
        { id: PLAYER_2, alignment: Alignment.Ringwraith, companies: [{ site: MORIA, characters: [ELWEN] }], hand: [], siteDeck: [DOL_GULDUR] },
      ],
    });
    const state: GameState = { ...base, turnNumber: 3, phaseState: makeSitePhase() };
    const elwenId = findCharInstanceId(state, HAZARD_PLAYER, ELWEN);

    const attempt = firstOpponentInfluenceAttempt(state, elwenId);
    ...
    // Nimloth base DI 1 + 2 (vs an elf) = 3.
  ...
  test('no +2 DI on an opponent-influence attempt against a non-elf character', () => {
    ...
    const pending = result.state.pendingResolutions.find(r => r.kind.type === 'opponent-influence-defend');
    if (pending?.kind.type !== 'opponent-influence-defend') throw new Error('no opponent-influence-defend pending');
    // Non-elf target → no bonus: base DI 1.
  ...
  test('+2 DI vs Elf Factions applies to an opponent-influence attempt against an elf faction', () => {
    ...
    // Thranduil's Halls, so Nimloth's company sits there.
    const base = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.Site,
      recompute: true,
      players: [
        { id: PLAYER_1, alignment: Alignment.Ringwraith, companies: [{ site: THRANDUILS_HALLS, characters: [NIMLOTH] }], hand: [], siteDeck: [DOL_GULDUR] },
    ...
    const pending = result.state.pendingResolutions.find(r => r.kind.type === 'opponent-influence-defend');
    if (pending?.kind.type !== 'opponent-influence-defend') throw new Error('no opponent-influence-defend pending');
    // Elf faction target → +2: base DI 1 + 2 = 3.
    expect(pending.kind.attempt.influencerDI).toBe(3);
  });
  ...
  test('no +2 DI on an opponent-influence attempt against a non-elf faction', () => {
    ...
    const pending = result.state.pendingResolutions.find(r => r.kind.type === 'opponent-influence-defend');
    if (pending?.kind.type !== 'opponent-influence-defend') throw new Error('no opponent-influence-defend pending');
    // Non-elf faction → no bonus: base DI 1.
    expect(pending.kind.attempt.influencerDI).toBe(1);
  });

  // ── Rule: "Agent only: may move to a Haven [{H}]." ─────────────────────────

  test('Nimloth-as-agent at Amon Hen may move to a Haven (Minas Morgul) as well as a non-haven (Edoras)', () => {
    ...
    // move to a haven (rule 9.07); Nimloth's exemption keeps Minas Morgul legal.
    const base = buildTestState({
      activePlayer: PLAYER_1,
    ...
    const amonHenStackSite: SiteInPlay = {
      instanceId: 'nimloth-start-amonhen' as CardInstanceId,
      definitionId: AMON_HEN,
      status: CardStatus.Untapped,
    };
    const agentChar: CharacterInPlay = {
      instanceId: 'nimloth-agent-char' as CardInstanceId,
      definitionId: NIMLOTH,
      status: CardStatus.Untapped,
    ...
    const agent: AgentInPlay = {
      id: 'agent-nimloth-0' as CompanyId,
      character: agentChar,
      revealed: true,
    ...
    // The haven destination is legal thanks to Nimloth's exemption...
