 ...
 * @module tw-051.test
 *
 * Card test: Lost in Border-lands (tw-51)
 ...
 *      scope:turn, target:target-company, regionType:border, value:1, floor:0
 *
 * Text:
 *   "Playable on a moving company. Its hazard limit increases by one for
 *    every Border-land [{b}] in its site path."
 *
 ...
 * `resolvedSitePath` — engine work newly added for this card (previously
 * that live path only handled `hazard-limit-modifier`; region-count
 * constraints were only consulted at `snapshotHazardLimit` time).
 *
 * "Playable on a moving company" (no minimum Border-land count, unlike Lost
 ...
 * | 2 | hazard limit increases by one per Border-land in path | IMPLEMENTED | hazard-limit-region-count constraint, read live in currentHazardLimit |
 *
 * Playable: YES
...
const LOST_IN_BORDER_LANDS = 'tw-51' as CardDefinitionId;

// Standard setup: resource company (P1) at Rivendell, hazard player (P2) at
// Lorien holding Lost in Border-lands only.
const LIBL_PLAYERS = [
  { id: PLAYER_1, companies: [{ site: RIVENDELL, characters: [ARAGORN] }], hand: [], siteDeck: [MORIA] },
  { id: PLAYER_2, companies: [{ site: LORIEN, characters: [LEGOLAS] }], hand: [LOST_IN_BORDER_LANDS], siteDeck: [MINAS_TIRITH] },
] as const;
...
describe('Lost in Border-lands (tw-51)', () => {
  beforeEach(() => resetMint());

  test('playable on a moving company with a Border-land in its site path', () => {
    const state = mhStateWithPath([RegionType.Border], SiteType.BorderHold);
    const actions = viableActionsForHandCard(
      state, PLAYER_2, 'play-hazard', HAZARD_PLAYER, LOST_IN_BORDER_LANDS,
    );
  ...
  test('playable on a moving company with no Border-land in its site path (no minimum required)', () => {
  ...
  test('playing it through reduce adds a hazard-limit-region-count constraint to the target company', () => {
    const state = mhStateWithPath([RegionType.Border, RegionType.Wilderness], SiteType.BorderHold);
    ...
    expect(inDiscard!.definitionId).toBe(LOST_IN_BORDER_LANDS);
  ...
  test('the live hazard limit increases by one per Border-land in the resolved site path', () => {
    const state = mhStateWithPath(
      [RegionType.Border, RegionType.Wilderness, RegionType.Border],
      SiteType.BorderHold,
    );
    const targetCompanyId = companyIdAt(state, RESOURCE_PLAYER);
    const cardId = handCardId(state, HAZARD_PLAYER);
    ...
    // Two Border-lands in the path → +2 to the live limit.
    expect(after).toBe(before + 2);
  });

  test('no increase to the live hazard limit when the site path has no Border-land', () => {
