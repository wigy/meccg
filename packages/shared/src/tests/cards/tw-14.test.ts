 ...
 * @module tw-14.test
 *
 * Card test: Barrow-wight (tw-14)
 * Type: hazard-creature
 * Effects: 1 (on-event: character-wounded-by-self → force corruption check -2)
 *
 * Second printing of Barrow-wight (see also tw-015, identical text/stats):
...
const BARROW_WIGHT_2 = 'tw-14' as CardDefinitionId;
const SHADOW_KEYING = { method: 'region-type' as const, value: 'shadow' };

// ─── Tests ───────────────────────────────────────────────────────────────────

describe('Barrow-wight (tw-14)', () => {
  ...
  test('combat initiates with 1 strike and 12 prowess', () => {
    const state = buildTestState({
      ...
      players: [
        ...
        { id: PLAYER_2, companies: [{ site: LORIEN, characters: [LEGOLAS] }], hand: [BARROW_WIGHT_2], siteDeck: [RIVENDELL] },
      ],
    });
