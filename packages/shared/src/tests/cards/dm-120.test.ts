 ...
 * no tap ability). In the reported game, Arwen's own "+7 direct influence
 * only usable against Aragorn II" (tw-122) plus this card's +2 direct
 * influence must total 9 — exactly matching Aragorn II's mind (9) — so she
 * can take control of him as a follower (CoE 2.II.3.2); without the card's
 * bonus her available DI topped out at 7, one short.
 *
 * Modeling follows the established templates: Gandalf's Friend (wh-98) for
 * the character-target + company-context + stat-modifier shape, and
 * Mistress Lobelia (dm-178) / Strider (ba-1) for the tap-to-fetch and
 * bearer-company-moves discard shape.
 */
...
describe('Choice of Lúthien (dm-120)', () => {
  ...
  // ─── Bug report: control of Aragorn II via the combined DI bonus ──────────
  ...
  test('Arwen CAN control Aragorn II with Choice of Lúthien attached (available DI 2+7=9 meets his mind 9)', () => {
    const withCard = attachItemToChar(controlCompany(), RESOURCE_PLAYER, ARWEN, CHOICE_OF_LUTHIEN);
