 ...
 *   - WELLINGHALL (as-170): free-hold, no gold-ring playable   ← invalid (no gold-ring)
...
describe('Bright Gold Ring (le-303)', () => {
  ...
  test('not offered at Wellinghall (free-hold without gold-ring)', () => {
    const base = buildTestState({
      activePlayer: PLAYER_1,
      recompute: true,
      players: [
        { id: PLAYER_1, companies: [{ site: WELLINGHALL, characters: [GORBAG] }], hand: [], siteDeck: [ETTENMOORS] },
        { id: PLAYER_2, companies: [{ site: DOL_GULDUR, characters: [SHAGRAT] }], hand: [], siteDeck: [MINAS_MORGUL] },
      ],
      phase: Phase.Site,
    });
