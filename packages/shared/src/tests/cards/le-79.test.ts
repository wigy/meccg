 ...
 * @module le-79.test
 *
 * Card test: Huorn (le-79)
 * Type: hazard-creature (Awakened Plant)
 * Effects: 0 (structural keying only)
 *
 * Twin card of Huorn (tw-45) — identical text, stats, and keying; only the
 * set printing differs. See tw-45.test.ts for the original coverage this
 * mirrors.
 *
 * Text:
 ...
 * either. Entry 4 is the region-scoped site-type keying: a `siteTypes`
 * restriction gated by a `when: { "destinationSite.region": { "$in": [...] } }`
 * clause, narrower than the ungated "or at sites in these regions" shape
 * used elsewhere, which prints no site-type restriction.
 *
 * Fixtures reuse the TW-printed hero-site copies of the named sites (as
 * tw-45's own test does), since the DSL keying matches by site *name* and
 * alignment, not by which set printed the card, and LE does not reprint
 * Old Forest / Wellinghall / Sarn Goriwing / Rhosgobel under hero alignment.
 *
 * Playable: YES
 */
...
describe('Huorn (le-79)', () => {
  ...
  test('keyable via site-type at Sarn Goriwing (Shadow-hold in Heart of Mirkwood) even without the region on the movement path', () => {
    ...
    // only passes through the region-gated siteTypes entry, not the plain
    // regionNames entry.
    const state = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.MovementHazard,
