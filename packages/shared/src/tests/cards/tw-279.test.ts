 ...
 * @module tw-279.test
 *
 * Card test: Men of Lamedon (tw-279)
...
describe('Men of Lamedon (tw-279)', () => {
  beforeEach(() => resetMint());

  test('+1 influence check bonus when bearer is dunadan', () => {
    // Faramir (dunadan, base DI 1) attempts to influence Men of Lamedon at Vale of Erech.
    // Men of Lamedon influence number = 8, Dúnadan get +1 check modifier from faction card.
  ...
  test('no bonus when bearer is non-dunadan', () => {
    // Legolas (elf, base DI 2) attempts to influence Men of Lamedon at Vale of Erech.
    // Men of Lamedon influence number = 8, Dúnadan check modifier does NOT apply to elves.
    //   modifier = DI 2
    //   need = 8 - 2 = 6
    const state = buildSitePhaseState({
      characters: [LEGOLAS],
