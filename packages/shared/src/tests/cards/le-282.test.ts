 ...
 *  Standard Modifications: Orcs of Gorgoroth (+2), Orcs of the Red Eye (-2)."
 *
...
describe('Orcs of Udûn (le-282)', () => {
  ...
  test('influence-attempt is legal at Cirith Gorgor with baseline need = 9 (DI 0)', () => {
    const state = buildMinionSitePhaseState({ site: CIRITH_GORGOR, characters: [GRISHNAKH], hand: [ORCS_OF_UDUN] });
  ...
  test('+2 modifier applies when controller also has Orcs of Gorgoroth in play', () => {
    ...
    const base = buildTestState({
      ...
      players: [
        { id: PLAYER_1, companies: [{ site: CIRITH_GORGOR, characters: [GRISHNAKH] }], hand: [ORCS_OF_UDUN], siteDeck: [CARN_DUM], cardsInPlay: [gorg] },
  ...
  test('-2 modifier applies when controller has Orcs of the Red Eye in play', () => {
    const redEye: CardInPlay = { instanceId: 're-1' as CardInstanceId, definitionId: ORCS_OF_THE_RED_EYE, status: CardStatus.Untapped };
    const base = buildTestState({
      activePlayer: PLAYER_1, phase: Phase.Site, recompute: true,
      players: [
        { id: PLAYER_1, companies: [{ site: CIRITH_GORGOR, characters: [GRISHNAKH] }], hand: [ORCS_OF_UDUN], siteDeck: [CARN_DUM], cardsInPlay: [redEye] },
        { id: PLAYER_2, companies: [{ site: CARN_DUM, characters: [LAGDUF] }], hand: [], siteDeck: [CARN_DUM] },
      ],
    });
    const state = { ...base, phaseState: makeSitePhase() };
  ...
  test('+2 modifier does NOT apply when only the OPPONENT has Orcs of Gorgoroth (per-player)', () => {
    const gorg: CardInPlay = { instanceId: 'gor-1' as CardInstanceId, definitionId: ORCS_OF_GORGOROTH, status: CardStatus.Untapped };
    const base = buildTestState({
      activePlayer: PLAYER_1, phase: Phase.Site, recompute: true,
      players: [
        { id: PLAYER_1, companies: [{ site: CIRITH_GORGOR, characters: [GRISHNAKH] }], hand: [ORCS_OF_UDUN], siteDeck: [CARN_DUM] },
  ...
  test('faction is NOT influenceable at a non-playable site (Goblin-gate)', () => {
    const state = buildMinionSitePhaseState({ site: GOBLIN_GATE, characters: [GRISHNAKH], hand: [ORCS_OF_UDUN] });
    const factionInstanceId = state.players[0].hand[0].instanceId;
  ...
  test('an Orc Leader is offered the leader-control influence variant', () => {
    const state = buildMinionSitePhaseState({ site: CIRITH_GORGOR, characters: [ORC_CAPTAIN], hand: [ORCS_OF_UDUN] });
  ...
  test('opponent can re-influence Orcs of Udûn while in play; value = 0', () => {
    const factionInPlay: CardInPlay = { instanceId: 'ou-1' as CardInstanceId, definitionId: ORCS_OF_UDUN, status: CardStatus.Untapped };
    const base = buildTestState({
      activePlayer: PLAYER_1, phase: Phase.Site, recompute: true,
      players: [
