 ...
 * @module tw-214.test
 *
 * Card test: Dwarven Ring of Bávor's Tribe (tw-214)
 ...
 *  modified by +2."
 *
 * Printed attributes (data/cards.json TW-214): corruption 3(5), marshalling
 ...
 * certified on Dwarven Ring of Durin's Tribe (tw-216).
 *
 * Engine Support:
 * | # | Rule                                                 | Status      | Notes                                                     |
 * |---|-------------------------------------------------------|-------------|-------------------------------------------------------------|
 ...
 * | 5 | Tap a Dwarf bearer to search play deck for a greater  | IMPLEMENTED | grant-action tw214-search-item, cost tap:bearer,             |
...
const DWARVEN_RING = 'tw-214' as CardDefinitionId;
...
function ringGrants(state: ReturnType<typeof siteStateWithRing>) {
  return viableActions(state, PLAYER_1, 'activate-granted-action')
    ...
    .filter(a => a.actionId === 'tw214-search-item');
}

describe("Dwarven Ring of Bávor's Tribe (tw-214)", () => {
