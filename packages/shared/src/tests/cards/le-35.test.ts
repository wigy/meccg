 ...
 * @module le-35.test
 *
 * Card test: Orc Veteran (le-35)
 * Type: minion-character (ringwraith alignment)
 * Stats: Orc warrior, prowess 4, body 8, mind 2, DI 0, MP 0
 ...
 * |---|------------------------------------|-------------|------------------------------------------------|
 * | 1 | discardBodyCheck [8] (structural)  | IMPLEMENTED | orc discarded to discard pile on failed body check |
 * | 2 | -1 corruption check modifier       | IMPLEMENTED | check-modifier effect, value -1                 |
 *
 * Playable: YES
 *
 * Rules exercised:
 * 1. discardBodyCheck [8]: orc is discarded when body check roll < effective threshold (8 base).
...
describe('Orc Veteran (le-35)', () => {
  ...
  test('-1 corruption modifier increases need on pending corruption check', () => {
      ...
      players: [
        {
          ...
          companies: [{ site: DOL_GULDUR, characters: [{ defId: ORC_VETERAN, items: [RED_BOOK] }] }],
          hand: [],
          siteDeck: [MORIA],
        },
        {
    ...
    const vetId = findCharInstanceId(state, RESOURCE_PLAYER, ORC_VETERAN);
    const itemInstId = getCharacter(state, RESOURCE_PLAYER, ORC_VETERAN).items[0].instanceId;

    const stateWithCheck = enqueueTransferCorruptionCheck(state, PLAYER_1, vetId, itemInstId);
    ...
    expect(ccActions[0].characterId).toBe(vetId);
  ...
  test('Orc Veteran is discarded when body check fails (roll < effective threshold)', () => {
    ...
    const state = buildTestState({
      ...
      activePlayer: PLAYER_1,
      players: [
        { id: PLAYER_1, companies: [{ site: DOL_GULDUR, characters: [ORC_VETERAN] }], hand: [], siteDeck: [MORIA] },
    ...
    const vetId = findCharInstanceId(s, RESOURCE_PLAYER, ORC_VETERAN);
    ...
    expectCharNotInPlay(s, RESOURCE_PLAYER, vetId);
    const discardDefIds = s.players[0].discardPile.map(c => c.definitionId);
    expect(discardDefIds).toContain(ORC_VETERAN);
  });

  test('Orc Veteran stays in play when body check passes (roll >= effective threshold)', () => {
