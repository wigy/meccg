 ...
 * @module le-380.test
 *
 * Card test: Haudh-in-Gwanûr (le-380)
 ...
 *
 * This is a pure twin of Barrow-downs (le-353): the same {s}{w}{w} sitePath and
 * the same Undead automatic-attack whose wounds force a corruption check modified
 * by -2. The only differences are cosmetic/structural — the auto-attack is at
 * prowess 10 (not 8) and the nearest Darkhaven is Minas Morgul (not Carn Dûm).
 * Both share the certified engine machinery: a site automatic-attack resolves its
 * `on-event: character-wounded-by-self` effect in combat-finalize.ts, enqueuing a
 * corruption check per wounded character.
 *
 ...
 * | 6 | automaticAttacks  | OK     | Undead, 1 strike, 10 prowess                      |
...
const HAUDH = 'le-380' as CardDefinitionId;
...
describe('Haudh-in-Gwanûr (le-380)', () => {
  beforeEach(() => resetMint());

  // ─── Automatic attack ──────────────────────────────────────────────────────

  test('Undead automatic attack triggers with 1 strike and 10 prowess', () => {
    const state = buildMinionSitePhaseState({ site: HAUDH, characters: [{ defId: THE_MOUTH }] });
  ...
  test('wounded character gets a corruption check modified by -2 after the auto-attack', () => {
    ...
    // Untapped: prowess 6-3=3, roll 2 → 2+3=5 < 10 → wounded. Body check vs the
    ...
    expect(pending[0].kind.reason).toBe('Haudh-in-Gwanûr');
  ...
  test('minor and major items are playable at Haudh-in-Gwanûr', () => {
    const state = buildMinionSitePhaseState({
      site: HAUDH,
  ...
  test('greater items are NOT playable at Haudh-in-Gwanûr', () => {
    const state = buildMinionSitePhaseState({
      site: HAUDH,
      characters: [{ defId: THE_MOUTH }],
      hand: [BLACK_MACE],
  ...
  test('starter movement from Minas Morgul reaches Haudh-in-Gwanûr', () => {
    ...
    expect(starterNames).toContain('Haudh-in-Gwanûr');
  });

  test('starter movement from Haudh-in-Gwanûr returns to Minas Morgul (nearest Darkhaven)', () => {
    const haudh = pool[HAUDH as string] as SiteCard;
    const allSites = Object.values(pool).filter(isSiteCard);
    const movementMap = buildMovementMap(pool);
  ...
  test('not reachable from Carn Dûm via starter movement', () => {
    ...
    expect(starterNames).not.toContain('Haudh-in-Gwanûr');
  });
});
