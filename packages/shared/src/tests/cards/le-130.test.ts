 ...
 * @module le-130.test
 *
 * Card test: Plague of Wights (le-130)
...
const PLAGUE_OF_WIGHTS = 'le-130' as CardDefinitionId;
// Dead Marshes (tw-384): Undead auto-attack — 2 strikes, 8 prowess.
const DEAD_MARSHES = 'tw-384' as CardDefinitionId;

describe('Plague of Wights (le-130)', () => {
  beforeEach(() => resetMint());

  const plagueInPlay: CardInPlay = {
    instanceId: 'plague-1' as CardInstanceId,
    definitionId: PLAGUE_OF_WIGHTS,
  ...
  test('cannot be duplicated (duplication-limit scope game max 1)', () => {
    const state = buildTestState({
      ...
      players: [
        ...
        { id: PLAYER_2, companies: [{ site: LORIEN, characters: [LEGOLAS] }], hand: [PLAGUE_OF_WIGHTS], siteDeck: [MINAS_TIRITH], cardsInPlay: [plagueInPlay] },
      ],
    });
