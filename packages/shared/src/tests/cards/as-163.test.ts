 ...
 * @module as-163.test
 *
 * Card test: The Under-courts (as-163)
 * Type: minion-site (dark-hold, under-deeps) in Gorgoroth — the Ringwraith-player
 * twin of the Balrog version (ba-98), but printed with a simpler text: no dynamic
 * second automatic-attack and no "attacks normally" Special, just a single
 * detainment automatic-attack.
 *
 * Text:
 ...
 *     authoritative): Barad-dûr (0), The Sulfur-deeps (4), The Under-galleries (4).
 *   - `combat-detainment` (unfiltered) — the printed "(detainment)" marker on the
 *     automatic-attack, unconditional for any defender (as-152/as-161 precedent).
 *
 ...
 * | 10| adjacentSites     | OK     | 3 entries — added this pass                    |
...
const THE_UNDER_COURTS = 'as-163' as CardDefinitionId;    // this card (minion under-deeps dark-hold)
const BARAD_DUR = 'le-352' as CardDefinitionId;           // Darkhaven — the surface site (roll 0)
const THE_SULFUR_DEEPS = 'as-161' as CardDefinitionId;    // minion under-deeps dark-hold (roll 4)
const THE_UNDER_GALLERIES = 'as-164' as CardDefinitionId; // minion under-deeps dark-hold (roll 4)
...
function anyState(): GameState {
  return buildTestState({
    ...
    players: [
      { id: PLAYER_1, alignment: Alignment.Ringwraith, companies: [{ site: BARAD_DUR, characters: [THE_MOUTH] }], hand: [], siteDeck: [] },
...
describe('The Under-courts (as-163)', () => {
  beforeEach(() => resetMint());

  // ─── Under-deeps adjacency and movement rolls ──────────────────────────────

  test('adjacent to its surface site Barad-dûr; descent and ascent both require roll 0', () => {
    ...
    expect(isUnderDeepsAdjacent(state, baradDur, underCourts)).toBe(true);
    // Descent (surface origin): always roll 0. Ascent: the printed Barad-dûr (0).
    expect(getUnderDeepsRequiredRoll(state, baradDur, underCourts)).toBe(0);
    expect(getUnderDeepsRequiredRoll(state, underCourts, baradDur)).toBe(0);
  });

  test('adjacent to The Sulfur-deeps and The Under-galleries at roll 4 each', () => {
    const state = anyState();
    const underCourts = pool[THE_UNDER_COURTS as string] as SiteCard;
    ...
    expect(isUnderDeepsAdjacent(state, underCourts, sulfurDeeps)).toBe(true);
    expect(getUnderDeepsRequiredRoll(state, underCourts, sulfurDeeps)).toBe(4);
    expect(isUnderDeepsAdjacent(state, underCourts, underGalleries)).toBe(true);
    expect(getUnderDeepsRequiredRoll(state, underCourts, underGalleries)).toBe(4);
  });
  ...
  test('NOT adjacent to an unlisted Darkhaven (Geann a-Lisch)', () => {
    ...
    expect(isUnderDeepsAdjacent(state, underCourts, geann)).toBe(false);
    expect(isUnderDeepsAdjacent(state, geann, underCourts)).toBe(false);
  });

  test('plan-movement offers descent from Barad-dûr to The Under-courts', () => {
    const state = orgAt(BARAD_DUR, [THE_UNDER_COURTS]);
  ...
  test('plan-movement offers moving on from The Under-courts to The Sulfur-deeps', () => {
    const state = orgAt(THE_UNDER_COURTS, [THE_SULFUR_DEEPS]);
  ...
  test('plan-movement does NOT offer The Under-courts from a non-adjacent Darkhaven (Geann a-Lisch)', () => {
    ...
    const state = orgAt(GEANN_A_LISCH, [THE_UNDER_COURTS]);
    const underCourtsInst = state.players[0].siteDeck.find(s => s.definitionId === THE_UNDER_COURTS)!.instanceId;
    const moves = viableFor(state, PLAYER_1)
      .filter(a => a.action.type === 'plan-movement') as { action: PlanMovementAction }[];
    expect(moves.some(a => a.action.destinationSite === underCourtsInst)).toBe(false);
  });

  // ─── Automatic attack: Trolls — 3 strikes with 10 prowess (detainment) ──────

  test('minion company entering The Under-courts faces the Trolls attack as detainment', () => {
    const state = buildMinionSitePhaseState({ site: THE_UNDER_COURTS, characters: [{ defId: THE_MOUTH }] });
  ...
  test('a minor item is playable at The Under-courts but a major item is not', () => {
