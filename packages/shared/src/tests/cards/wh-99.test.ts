...
describe('Give Welcome to the Unexpected (wh-99)', () => {
  ...
  // Bug report: "Give Welcome to the Unexpected is not playable during
  // Organisation Phase if Gandalf IS not in play (should be and to be put on
  // Gandalf when he enters play)." Mirrors Bade to Rule (le-167)'s alternative
  // untargeted mode + `on-event: avatar-enters-play` auto-attach.
  ...
  test('alternative mode: playable during the organization phase if Gandalf is not in play', () => {
    ...
    expect(after.players[RESOURCE_PLAYER].cardsInPlay.some(c => c.definitionId === GIVE_WELCOME)).toBe(true);
  ...
  test('when played bare, attaches to Gandalf the moment he enters play', () => {
    ...
    expect(bare.players[RESOURCE_PLAYER].cardsInPlay.some(c => c.definitionId === GIVE_WELCOME)).toBe(true);

    // A second copy must not be offered while the first sits bare in play.
    const secondCopyActions = viableActions(addCardToHand(bare, RESOURCE_PLAYER, GIVE_WELCOME), PLAYER_1, 'play-permanent-event');
    ...
    expect(withGandalf.players[RESOURCE_PLAYER].cardsInPlay.some(c => c.definitionId === GIVE_WELCOME)).toBe(false);
    const gandalf = getCharacter(withGandalf, RESOURCE_PLAYER, GANDALF_FW);
    expect(gandalf.items.some(i => i.definitionId === GIVE_WELCOME)).toBe(true);
    expect(withGandalf.players[RESOURCE_PLAYER].stagePoints).toBe(2);
  });
});
