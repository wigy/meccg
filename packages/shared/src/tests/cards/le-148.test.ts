 ...
 *
 * le-148 is a Lidless Eye reprint of Wake of War (tw-108, The Wizards) with
 * identical text and effects — see tw-108.test.ts for the original. The
 * duplication-limit is checked by card name (`countCopiesInPlay`), so the two
 * printings share the same "cannot be duplicated" pool across sets.
 */
...
const WAKE_OF_WAR = 'le-148' as CardDefinitionId;
const WAKE_OF_WAR_TW = 'tw-108' as CardDefinitionId;

// ─── Tests ─────────────────────────────────────────────────────────────────

describe('Wake of War (le-148)', () => {
  beforeEach(() => resetMint());

  const wowInPlay: CardInPlay = {
    instanceId: 'wow-le-1' as CardInstanceId,
  ...
  test('Wolves get +2 prowess and strikes with Doors of Night in play', () => {
    ...
    const donInPlay: CardInPlay = {
      instanceId: 'don-le-1' as CardInstanceId,
  ...
  test('cannot be duplicated across printings: tw-108 in play blocks le-148 from hand', () => {
    // The two printings share a name, so the duplication-limit pool (checked
    // by card name via countCopiesInPlay) is shared across sets.
    const twInPlay: CardInPlay = {
      instanceId: 'wow-tw-1' as CardInstanceId,
      definitionId: WAKE_OF_WAR_TW,
      status: CardStatus.Untapped,
    };
    const state = buildTestState({
      activePlayer: PLAYER_1,
      ...
      players: [
        { id: PLAYER_1, companies: [{ site: RIVENDELL, characters: [ARAGORN] }], hand: [], siteDeck: [MORIA] },
        { id: PLAYER_2, companies: [{ site: LORIEN, characters: [LEGOLAS] }], hand: [WAKE_OF_WAR], siteDeck: [MINAS_TIRITH], cardsInPlay: [twInPlay] },
      ],
    });
