 ...
 *
 * Base stats: strikes 2, prowess 15, body 9, kill MP 4.
 *
 * keyedTo:
 * | # | Entry                                                                                  | When                        |
 * |---|----------------------------------------------------------------------------------------|-----------------------------|
 * | 1 | siteNames: [Caves of Ûlund]                                                            | always                      |
...
const AGBURANAR = 'tw-3' as CardDefinitionId;

/** MH state simulating arrival at Caves of Ûlund (no DoN needed). */
function mhAtCavesOfUlund(): ReturnType<typeof makeMHState> {
  return makeMHState({
    ...
    destinationSiteName: 'Caves of Ûlund',
  });
}
...
describe('Agburanar (tw-3)', () => {
  beforeEach(() => resetMint());

  // ─── Combat: 2 strikes, 15 prowess ──────────────────────────────────────

  test('combat starts with normal assignment, 2 strikes at 15/9', () => {
    ...
    const ready: GameState = { ...state, phaseState: mhAtCavesOfUlund() };

    const agburanarId = handCardId(ready, HAZARD_PLAYER);
    const companyId = companyIdAt(ready, RESOURCE_PLAYER);

    const afterChain = playCreatureHazardAndResolve(
      ready, PLAYER_2, agburanarId, companyId,
      { method: 'site-name' as const, value: 'Caves of Ûlund' },
    );
    ...
    // since Agburanar carries no attacker-chooses-defenders effect.
    expect(viableActions(afterChain, PLAYER_1, 'assign-strike').length).toBeGreaterThan(0);
  });

  // ─── Keying: at Caves of Ûlund, no DoN required ─────────────────────────

  test('keyable at Caves of Ûlund without Doors of Night', () => {
    const state = buildTestState({
      activePlayer: PLAYER_1,
      phase: Phase.MovementHazard,
      recompute: true,
    ...
    expect(plays.some(p => {
      ...
      return a.keyedBy?.method === 'site-name' && a.keyedBy?.value === 'Caves of Ûlund';
    })).toBe(true);
  });
  ...
  test('keyable at any site in Northern Rhovanion with Doors of Night (site-in-region clause)', () => {
    // Destination site is a non-Caves-of-Ûlund site in Northern Rhovanion.
  ...
  test('NOT keyable to an unrelated region even with Doors of Night', () => {
    ...
    // Rhudaur wilderness path to Moria — none of Agburanar's four regions.
