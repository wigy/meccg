 ...
 * @module tw-396.test
 *
 * Card test: Gladden Fields (tw-396)
 * Type: hero-site (ruins-and-lairs)
 * Effects: 1 (on-event: character-wounded-by-self → force corruption check)
 *
 * "Nearest Haven: Lórien Playable: Items (gold ring) Automatic-attacks:
 *  Undead — 1 strike with 8 prowess; each character wounded must make a
 *  corruption check."
 *
 ...
 * | 2 | sitePath          | OK     | wilderness, border — matches card ({w}{b})    |
 * | 3 | nearestHaven      | OK     | "Lórien" — valid haven in card pool           |
 * | 4 | playableResources | OK     | gold-ring — matches card text                 |
 ...
 * | 2 | Item playability              | IMPLEMENTED | gold-ring checked                   |
...
describe('Gladden Fields (tw-396)', () => {
  beforeEach(() => resetMint());

  // ─── Automatic attack ──────────────────────────────────────────────────────

  test('Undead automatic attack triggers with 1 strike and 8 prowess', () => {
    const state = buildSitePhaseState({ site: GLADDEN_FIELDS });
  ...
  test('gold ring items playable at Gladden Fields', () => {
    const state = buildSitePhaseState({
      site: GLADDEN_FIELDS,
  ...
  test('starter movement from Lórien reaches Gladden Fields', () => {
    ...
    expect(starterNames).toContain('Gladden Fields');
  });

  test('starter movement from Gladden Fields reaches Lórien (back to nearest haven)', () => {
    const gladdenFields = pool[GLADDEN_FIELDS as string] as SiteCard;
    const allSites = Object.values(pool).filter(isSiteCard);
    const movementMap = buildMovementMap(pool);

    const reachable = getReachableSites(movementMap, gladdenFields, allSites);
    const starterNames = reachable
      .filter(r => r.movementType === 'starter')
      .map(r => r.site.name);
