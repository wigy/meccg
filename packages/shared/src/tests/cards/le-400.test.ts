 ...
 * @module le-400.test
 *
 * Card test: Ruined Signal Tower (le-400)
 * Type: minion-site (ruins-and-lairs) in Enedhwaith
 *
 * Text:
 ...
 * with a single Spiders automatic-attack and minor/major item playability. Every
 * rule on the card routes through the standard site machinery already in the
 * engine (movement via nearestHaven, item playability via playableResources,
 * automatic-attack combat in reducer-site.ts).
 *
 * Site Structural Checks (documented; verified behaviourally below):
 * | # | Property          | Notes                                                       |
 * |---|-------------------|-------------------------------------------------------------|
 * | 1 | siteType          | "ruins-and-lairs" — valid                                   |
 * | 2 | sitePath          | [wilderness, wilderness] — matches {w}{w}                   |
 * | 3 | nearestHaven      | "Geann a-Lisch" — valid minion haven (le-374) in card pool  |
 * | 4 | region            | "Enedhwaith" — valid region in card pool                    |
 * | 5 | playableResources | [minor, major] — matches card text "Items (minor, major)"   |
 * | 6 | automaticAttacks  | Spiders — 2 strikes, 8 prowess                              |
 * | 7 | resourceDraws     | 1                                                           |
 * | 8 | hazardDraws       | 1                                                           |
 *
 * Engine Support:
 ...
 * | 4 | Haven path movement           | IMPLEMENTED | Geann a-Lisch ↔ Ruined Signal Tower via starter    |
 * | 5 | Spiders automatic-attack      | IMPLEMENTED | reducer-site.ts initiates 2×8 Spiders combat       |
 *
 * Playable: YES
...
const RUINED_SIGNAL_TOWER = 'le-400' as CardDefinitionId;
...
// Build a Ringwraith company at the Ruined Signal Tower sitting at the
// automatic-attacks step (the single Spiders attack is next to fire).
    ...
    players: [
      {
        ...
        companies: [{ site: RUINED_SIGNAL_TOWER, characters }],
        hand: [],
...
describe('Ruined Signal Tower (le-400)', () => {
  beforeEach(() => resetMint());

  // ─── Movement: Geann a-Lisch ↔ Ruined Signal Tower ──────────────────────────

  test('starter movement from Geann a-Lisch reaches Ruined Signal Tower', () => {
    ...
    const starter = reachable.find(
      r => r.movementType === 'starter' && r.site.id === (RUINED_SIGNAL_TOWER as string),
    );
    expect(starter).toBeDefined();
  });

  test('starter movement from Ruined Signal Tower returns to Geann a-Lisch', () => {
  ...
  test('starter movement from Lórien (hero haven) does NOT reach the minion Ruined Signal Tower', () => {
  ...
  test('haven-to-haven movement from Geann a-Lisch does not include the Ruined Signal Tower (not a haven)', () => {
    const geann = pool[GEANN_A_LISCH as string] as SiteCard;
    const havenLinks = buildMovementMap(pool).havenToHaven.get(geann.name);
    // Geann a-Lisch's only haven link is Carn Dûm; the tower is not a haven.
    expect(havenLinks?.has('Ruined Signal Tower') ?? false).toBe(false);
  });

  // ─── Item playability: minor and major, but not greater ─────────────────────

  test('minor item (Black-hide Shield) is playable at the Ruined Signal Tower', () => {
  ...
  test('major item (Sable Shield) is playable at the Ruined Signal Tower', () => {
    const state = buildMinionSitePhaseState({
      site: RUINED_SIGNAL_TOWER,
      characters: [GORBAG],
      hand: [SABLE_SHIELD],
  ...
  // ─── Automatic attack: Spiders — 2 strikes with 8 prowess ───────────────────

  test('automatic attack is Spiders — 2 strikes with 8 prowess, a normal (cancelable) attack', () => {
  ...
  test('the Spiders attack against a Ringwraith company at a Ruins & Lairs is NOT detainment (a real wounding attack)', () => {
    ...
    // (Orc/Troll/Undead/Man) is keyed to a Shadow-land. A Spiders attack keyed
    // to a Ruins & Lairs is neither, so it wounds normally.
    const towerDef = pool[RUINED_SIGNAL_TOWER as string] as SiteCard;
    const detainment = isDetainmentAttack({
      attackEffects: towerDef.effects,
      attackRace: Race.Spider,
      attackKeyedTo: [{ siteTypes: [SiteType.RuinsAndLairs] }],
      defendingAlignment: Alignment.Ringwraith,
      defendingSiteEffects: towerDef.effects,
    });
    expect(detainment).toBe(false);
  });

  test('the triggered Spiders combat is flagged as a non-detainment attack', () => {
    const triggered = dispatch(atAutoAttack([GORBAG, LAGDUF]), { type: 'pass', player: PLAYER_1 });
    expect(triggered.combat).not.toBeNull();
    expect(triggered.combat!.detainment).toBe(false);
  });

  test('the Spiders attack resolves to completion: both defenders survive a defeated strike', () => {
    // Two orcs each face one strike; both tap to fight and roll high enough to
    // defeat the 8-prowess strike. Combat finalizes with both still in play.
