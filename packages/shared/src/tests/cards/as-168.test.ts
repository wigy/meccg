 ...
 * @module as-168.test
 *
 * Card test: The Under-vaults (as-168)
 * Type: minion-site (ruins-and-lairs, under-deeps) in Angmar
 *
 * Text:
 ...
 *
 * Rules interpretation: as-168 is the Ringwraith (minion) twin of the hero
 * The Under-vaults (dm-41) and the Balrog The Under-vaults (ba-103), both
 * already certified. All three printed rules map to the same two engine
 * primitives certified on the twins — there is no minion-specific override
 * (unlike as-165, this card prints no "attacks normally, not as detainment"
 * clause, so none is encoded):
 *   - `site-rule: dynamic-auto-attack` keyed to Shadow-hold {S} — the 2nd
 *     automatic-attack (opponent plays a non-unique Shadow-hold-keyed hazard
 *     creature from hand as an auto-attack).
 *   - `site-rule: allow-creature-by-race` race "undead" — the Special: any
 *     Undead creature may be keyed to (played at) this site regardless of its
 *     printed keying.
 *
 ...
 *   - `automaticAttacks[0].creatureType` filled to "Undead" (was "").
 *   - `adjacentSites` (Mount Gram 0 / The Iron-deeps 6 / The Under-leas 6 —
 *     minion rolls per the printed line).
 *   - the two `effects` above (was `[]`).
 *
 ...
 * | 6 | automaticAttacks  | FIXED  | Undead, 3 strikes, 8 prowess (creatureType filled)      |
 * | 7 | resourceDraws     | OK     | 1                                                        |
 * | 8 | hazardDraws       | OK     | 2                                                        |
 * | 9 | keywords          | OK     | ["under-deeps"]                                          |
 * | 10| adjacentSites     | FIXED  | Mount Gram 0 / The Iron-deeps 6 / The Under-leas 6      |
 * | 11| effects           | FIXED  | dynamic-auto-attack (shadow-hold) + allow-creature-by-race (undead) |
 *
 * Engine Support:
...
const THE_UNDER_VAULTS = 'as-168' as CardDefinitionId;
const THE_UNDER_GROTTOS = 'ba-101' as CardDefinitionId; // ruins-and-lairs, under-deeps, dynamic-auto-attack only (NO allow-creature-by-race)
...
/** Minion company at The Under-vaults in the site phase, hazard hand configurable. */
function dualHandState(opts: {
  ...
  const state = buildTestState({
    ...
    phase: Phase.Site,
    players: [
      { id: PLAYER_1, alignment: Alignment.Ringwraith, companies: [{ site: THE_UNDER_VAULTS, characters: [THE_MOUTH] }], hand: [], siteDeck: [DOL_GULDUR] },
...
/** Movement/hazard state with a minion company at `site`, hazard player holding `hazardHand`. */
function mhAt(site: CardDefinitionId, siteName: string, hazardHand: CardDefinitionId[]): GameState {
  const state = buildTestState({
    activePlayer: PLAYER_1,
...
describe('The Under-vaults (as-168)', () => {
  ...
  test('Barrow-wight (Undead, normally keyed to shadow/dark) is viable against the company at The Under-vaults', () => {
    ...
    // (under-deeps) region path. Without the site rule, Barrow-wight would NOT
    // match. With allow-creature-by-race for "undead", the keying is bypassed.
    const mhState = mhAt(THE_UNDER_VAULTS, 'The Under-vaults', [BARROW_WIGHT]);
  ...
  test('Assassin (Men, NOT Undead) is NOT made viable by the site rule at The Under-vaults', () => {
    ...
    const mhState = mhAt(THE_UNDER_VAULTS, 'The Under-vaults', [ASSASSIN]);
  ...
  test('Barrow-wight is NOT made viable at a ruins-and-lairs under-deeps site without the site rule (The Under-grottos)', () => {
    // The Under-grottos (ba-101) is also a ruins-and-lairs under-deeps site but
    // carries no allow-creature-by-race rule. Barrow-wight is keyed to shadow-
    // hold/dark-hold, not ruins-and-lairs — so without the site rule it is not
    // viable here. This isolates allow-creature-by-race as the reason it IS
    // viable at The Under-vaults.
    const mhState = mhAt(THE_UNDER_GROTTOS, 'The Under-grottos', [BARROW_WIGHT]);
    const barrowWightInst = mhState.players[1].hand[0].instanceId;
    const all = computeLegalActions(mhState, PLAYER_2).filter(ea => ea.action.type === 'play-hazard');
