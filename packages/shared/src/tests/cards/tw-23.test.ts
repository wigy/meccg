...
describe('Corpse-candle (tw-23)', () => {
  ...
  test('company mate may tap in support (+1) of a pending corruption check (CoE 7.1.1)', () => {
    ...
    const supports = viableActions(afterPass, PLAYER_1, 'support-corruption-check')
    ...
    const baseModifier = (viableActions(afterPass, PLAYER_1, 'corruption-check')
    ...
    const afterSupport = dispatch(afterPass, supportAction);
