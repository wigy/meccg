 ...
 * @module tw-217.test
 *
 * Card test: Dwarven Ring of Dwálin's Tribe (tw-217)
 ...
 *  Dwarf bearer. Tap a Dwarf bearer to search your play deck for a major
 ...
 *  modified by +2."
 *
 * Printed attributes (data/cards.json TW-217): corruption 3(5), marshalling
 ...
 * | 5 | Tap a Dwarf bearer to search play deck for a major    | IMPLEMENTED | grant-action tw217-search-item, cost tap:bearer,             |
 ...
 * minor/major/greater/gold-ring items, so Durin's Axe (tw-212, subtype
 * "major") is playable there.
 *
 * Playable: YES
 */
...
const DWARVEN_RING = 'tw-217' as CardDefinitionId;
...
function ringGrants(state: ReturnType<typeof siteStateWithRing>) {
  return viableActions(state, PLAYER_1, 'activate-granted-action')
    ...
    .filter(a => a.actionId === 'tw217-search-item');
}

describe("Dwarven Ring of Dwálin's Tribe (tw-217)", () => {
  ...
  test('activating taps the bearer and enqueues a deck-only fetch to hand, playable at Moria, must-play-or-discard', () => {
    ...
    state = addCardToPlayDeck(state, RESOURCE_PLAYER, DURINS_AXE);
    ...
    // Only the major item (Durin's Axe) matches — deck has nothing else.
    ...
    const axeInstance = after.players[RESOURCE_PLAYER].playDeck.find(c => c.definitionId === DURINS_AXE)!;
    expect((fetchActions[0].action as { cardInstanceId: string }).cardInstanceId).toBe(axeInstance.instanceId as unknown as string);
  });
  ...
  test('the found item lands in hand and blocks every other action until played or discarded', () => {
    ...
    expect(after.players[RESOURCE_PLAYER].hand.some(c => c.definitionId === DURINS_AXE)).toBe(true);
    ...
    const axeId = findHandCardId(after, RESOURCE_PLAYER, DURINS_AXE);
    if (blocking && blocking.kind.type === 'play-or-discard-fetched-item') {
      expect(blocking.kind.cardInstanceId).toBe(axeId);
    }
    ...
    for (const ea of allViable) {
      ...
      expect((ea.action as { cardInstanceId?: unknown }).cardInstanceId).toBe(axeId);
    }
  ...
  test('playing the found item attaches it, clears the block, and enqueues the +2 corruption check on the bearer', () => {
    ...
    const playAction = computeLegalActions(after, PLAYER_1).find(
      ea => ea.viable && ea.action.type === 'play-hero-resource'
        && (ea.action as { cardInstanceId?: unknown }).cardInstanceId === axeId
    ...
    expect(afterPlay.players[RESOURCE_PLAYER].characters[aragornId].items
      .some(i => i.definitionId === DURINS_AXE)).toBe(true);
  ...
  test('discarding the found item instead of playing it clears the block and still enqueues the +2 corruption check', () => {
    ...
    expect(afterDiscard.players[RESOURCE_PLAYER].hand.some(c => c.definitionId === DURINS_AXE)).toBe(false);
    expectInDiscardPile(afterDiscard, RESOURCE_PLAYER, DURINS_AXE);
  ...
  test('an empty search still reshuffles and fires the corruption check immediately, with no blocking resolution', () => {
    ...
    // No major item in the deck — only `pass` is offered to skip the search.
