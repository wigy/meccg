...
describe('Éowyn (tw-147)', () => {
  ...
  test('+6 prowess bonus applies in combat vs nazgul (no tap)', () => {
    ...
    const ready = makeSingleCharCombatState({ heroDefId: EOWYN, creatureRace: Race.Ringwraith, creatureProwess: 15, creatureBody: 10 });
  ...
  test('nazgul body 9 (odd) is halved to 5 (rounded up) during body check', () => {
    ...
    const ready = makeSingleCharCombatState({
      heroDefId: EOWYN, creatureRace: Race.Ringwraith, creatureProwess: 5, creatureBody: 9, preAssigned: true,
    });
  ...
  test('nazgul body 10 (even) is halved to 5 during body check', () => {
    // Body 10 → ceil(10/2) = 5.
    const ready = makeSingleCharCombatState({
      heroDefId: EOWYN, creatureRace: Race.Ringwraith, creatureProwess: 5, creatureBody: 10, preAssigned: true,
    });
  ...
  test('enemy body is not halved vs non-nazgul', () => {
    ...
    const ready = makeSingleCharCombatState({
      heroDefId: EOWYN, creatureRace: Race.Orc, creatureProwess: 3, creatureBody: 9, preAssigned: true,
    });
