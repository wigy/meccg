 ...
 * @module td-70.test
 *
 * Card test: Smaug Ahunt (td-70)
 * Type: hazard-event (long, unique)
 * Effects: 2 (duplication-limit scope:game max:1, ahunt-attack)
 *
 * "Unique. Any company moving in Withered Heath, Northern Rhovanion, Iron Hills,
 *  and/or Grey Mountain Narrows faces one Dragon attack (considered a hazard
 *  creature attack) — 3 strikes at 15/7 (attacker chooses defending characters).
 ...
 * | 3 | Attack is a Dragon (3 str, 15/7)   | IMPLEMENTED | strikes/prowess/body on effect     |
...
// Base regions (trigger without Doors of Night)
...
describe('Smaug Ahunt (td-70)', () => {
  beforeEach(() => resetMint());

  test('company moving through Withered Heath triggers ahunt combat with correct stats', () => {
  ...
  test('company moving through Northern Rhovanion triggers ahunt combat', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: SMAUG_AHUNT, ...PATH_NORTHERN_RHOVANION });
  ...
  test('company moving through Iron Hills triggers ahunt combat', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: SMAUG_AHUNT, ...PATH_IRON_HILLS });
  ...
  test('company moving through Grey Mountain Narrows triggers ahunt combat', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: SMAUG_AHUNT, ...PATH_GREY_MOUNTAIN_NARROWS });
  ...
  test('company moving through non-matching region does not trigger ahunt', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: SMAUG_AHUNT, ...PATH_NON_MATCHING });
  ...
  test('non-moving company (empty path) does not trigger ahunt', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: SMAUG_AHUNT, ...PATH_EMPTY });

    const next = dispatch(state, viableActions(state, PLAYER_1, 'pass')[0].action);
    expect(next.combat).toBeNull();
  });

  // ─── Doors-of-Night-gated regions ──────────────────────────────────────────

  test('Brown Lands does NOT trigger without Doors of Night', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: SMAUG_AHUNT, ...PATH_BROWN_LANDS });
  ...
  test('Southern Rhovanion does NOT trigger without Doors of Night', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: SMAUG_AHUNT, ...PATH_SOUTHERN_RHOVANION });

    const next = dispatch(state, viableActions(state, PLAYER_1, 'pass')[0].action);
    expect(next.combat).toBeNull();
  });

  test('Dorwinion does NOT trigger without Doors of Night', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: SMAUG_AHUNT, ...PATH_DORWINION });

    const next = dispatch(state, viableActions(state, PLAYER_1, 'pass')[0].action);
    expect(next.combat).toBeNull();
  });

  test('Dagorlad does NOT trigger without Doors of Night', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: SMAUG_AHUNT, ...PATH_DAGORLAD });

    const next = dispatch(state, viableActions(state, PLAYER_1, 'pass')[0].action);
    expect(next.combat).toBeNull();
  });

  test('Horse Plains does NOT trigger without Doors of Night', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: SMAUG_AHUNT, ...PATH_HORSE_PLAINS });

    const next = dispatch(state, viableActions(state, PLAYER_1, 'pass')[0].action);
    expect(next.combat).toBeNull();
  });

  test('Brown Lands triggers with Doors of Night in play', () => {
    const state = buildAhuntOrderEffectsState({
      ahuntDefId: SMAUG_AHUNT,
      ...PATH_BROWN_LANDS,
  ...
  test('Dagorlad triggers with Doors of Night in play', () => {
    const state = buildAhuntOrderEffectsState({
      ahuntDefId: SMAUG_AHUNT,
      ...PATH_DAGORLAD,
      extraCardsInPlay: [DOORS_OF_NIGHT],
  ...
  test('Horse Plains triggers with Doors of Night in play', () => {
    const state = buildAhuntOrderEffectsState({
      ahuntDefId: SMAUG_AHUNT,
      ...PATH_HORSE_PLAINS,
      extraCardsInPlay: [DOORS_OF_NIGHT],
  ...
  test('ahunt long-event stays in cardsInPlay after combat resolves', () => {
    const state = buildAhuntOrderEffectsState({ ahuntDefId: SMAUG_AHUNT, ...PATH_WITHERED_HEATH });
    ...
    expect(current.players[1].cardsInPlay.some(c => c.definitionId === SMAUG_AHUNT)).toBe(true);
    expect(current.players[0].killPile.some(c => c.definitionId === SMAUG_AHUNT)).toBe(false);
    expect(current.players[1].discardPile.some(c => c.definitionId === SMAUG_AHUNT)).toBe(false);
  });
});
