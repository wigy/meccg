 ...
 * @module tw-338.test
 *
 * Card test: Test of Form (tw-338)
 * Type: hero-resource-event (short, alignment: wizard), non-unique, 0 MP
 *
 * "Sage only. Play to test a gold ring in a sage's company."
 *
 * Test of Form is the hero counterpart of Test of Fire (le-239): the active,
 * roll-based gold-ring test of Rule 6.2. Playing it runs the full test on a
 * chosen gold ring borne by a character in a sage's company — 2d6 (no roll
 * modifier), the tested gold ring's own `ring-test-table` maps the total to the
 * eligible ring categories, the gold ring is discarded regardless of result, and
 * an eligible special ring may immediately replace it on its former bearer.
 *
 * Unlike the Wizard tap-test (Gandalf tw-156's `test-gold-ring` granted action)
 * this route costs no tap at all — the sage merely authorizes the test.
 *
 * Engine support:
 * | # | Rule                                                   | Status      | Notes                                                        |
 * |---|--------------------------------------------------------|-------------|--------------------------------------------------------------|
...
const TEST_OF_FORM = 'tw-338' as CardDefinitionId;
