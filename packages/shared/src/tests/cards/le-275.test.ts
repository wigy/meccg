 ...
 * @module le-275.test
 *
 * Card test: Orcs of Gorgoroth (le-275)
...
const ORCS_OF_UDUN = 'le-282' as CardDefinitionId;     // +2 Standard Modification
...
describe('Orcs of Gorgoroth (le-275)', () => {
  beforeEach(() => resetMint());

  test('influence-attempt is legal at Barad-dûr with baseline need = 9 (DI 0)', () => {
    const state = buildMinionSitePhaseState({ site: BARAD_DUR, characters: [GRISHNAKH], hand: [ORCS_OF_GORGOROTH] });
  ...
  test('+2 modifier applies when controller also has Orcs of Udûn in play', () => {
    ...
    const base = buildTestState({
      ...
      players: [
        { id: PLAYER_1, companies: [{ site: BARAD_DUR, characters: [GRISHNAKH] }], hand: [ORCS_OF_GORGOROTH], siteDeck: [CARN_DUM], cardsInPlay: [udun] },
  ...
  test('-2 modifier applies when controller has Orcs of Mirkwood in play', () => {
    const mirkwood: CardInPlay = { instanceId: 'mw-1' as CardInstanceId, definitionId: ORCS_OF_MIRKWOOD, status: CardStatus.Untapped };
    const base = buildTestState({
      activePlayer: PLAYER_1, phase: Phase.Site, recompute: true,
      players: [
        { id: PLAYER_1, companies: [{ site: BARAD_DUR, characters: [GRISHNAKH] }], hand: [ORCS_OF_GORGOROTH], siteDeck: [CARN_DUM], cardsInPlay: [mirkwood] },
        { id: PLAYER_2, companies: [{ site: CARN_DUM, characters: [LAGDUF] }], hand: [], siteDeck: [CARN_DUM] },
      ],
    });
    const state = { ...base, phaseState: makeSitePhase() };
  ...
  test('+2 modifier does NOT apply when only the OPPONENT has Orcs of Udûn (per-player)', () => {
    const udun: CardInPlay = { instanceId: 'udun-1' as CardInstanceId, definitionId: ORCS_OF_UDUN, status: CardStatus.Untapped };
    const base = buildTestState({
      activePlayer: PLAYER_1, phase: Phase.Site, recompute: true,
      players: [
        { id: PLAYER_1, companies: [{ site: BARAD_DUR, characters: [GRISHNAKH] }], hand: [ORCS_OF_GORGOROTH], siteDeck: [CARN_DUM] },
        { id: PLAYER_2, companies: [{ site: CARN_DUM, characters: [LAGDUF] }], hand: [], siteDeck: [CARN_DUM], cardsInPlay: [udun] },
      ],
    });
    const state = { ...base, phaseState: makeSitePhase() };
    const factionInstanceId = state.players[0].hand[0].instanceId;
  ...
  test('faction is NOT influenceable at a non-playable site (Goblin-gate)', () => {
    const state = buildMinionSitePhaseState({ site: GOBLIN_GATE, characters: [GRISHNAKH], hand: [ORCS_OF_GORGOROTH] });
    const factionInstanceId = state.players[0].hand[0].instanceId;
  ...
  test('an Orc Leader is offered the leader-control influence variant', () => {
    const state = buildMinionSitePhaseState({ site: BARAD_DUR, characters: [ORC_CAPTAIN], hand: [ORCS_OF_GORGOROTH] });
  ...
  test('opponent can re-influence Orcs of Gorgoroth while in play; value = 0', () => {
    const factionInPlay: CardInPlay = { instanceId: 'og-1' as CardInstanceId, definitionId: ORCS_OF_GORGOROTH, status: CardStatus.Untapped };
    const base = buildTestState({
      activePlayer: PLAYER_1, phase: Phase.Site, recompute: true,
      players: [
