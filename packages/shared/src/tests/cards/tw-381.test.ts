 ...
 * @module tw-381.test
 *
 * Card test: Caves of Ûlund (tw-381)
 ...
 *
 * Site data marks this as the `lairOf` Agburanar (tw-3) with the `hoard`
 * keyword — structural metadata consumed by other cards (dragon playability
 * gates, hoard-item lookups) carrying no rules text of their own on this site.
 *
 * Site Structural Checks:
 * | # | Property          | Status | Notes                                                              |
 * |---|-------------------|--------|--------------------------------------------------------------------|
 * | 1 | siteType          | OK     | "ruins-and-lairs" — valid                                         |
 * | 2 | sitePath          | OK     | [wilderness, border, shadow, wilderness] — matches path tokens     |
 ...
 * | 6 | automaticAttacks  | OK     | Dragon, 1 strike, 13 prowess — matches card text                   |
 ...
 * | 6 | Automatic attack (1)     | IMPLEMENTED | Dragon 1×13 initiated with correct stats      |
 *
 * Playable: YES
...
describe('Caves of Ûlund (tw-381)', () => {
  beforeEach(() => resetMint());

  // ─── Site phase behavior ────────────────────────────────────────────────────

  test('minor items are playable at Caves of Ûlund', () => {
  ...
  test('major items are playable at Caves of Ûlund', () => {
    const state = buildSitePhaseState({
      site: CAVES_OF_ULUND,
      characters: [ARAGORN],
      hand: [GLAMDRING],
  ...
  test('greater items are playable at Caves of Ûlund', () => {
    const state = buildSitePhaseState({
      site: CAVES_OF_ULUND,
      characters: [ARAGORN],
  ...
  test('gold-ring items are playable at Caves of Ûlund', () => {
    const state = buildSitePhaseState({
      site: CAVES_OF_ULUND,
      characters: [ARAGORN],
  ...
  test('pass is always available during play-resources step', () => {
    const state = buildSitePhaseState({ site: CAVES_OF_ULUND });
  ...
  test('automatic attack: Dragon — 1 strike with 13 prowess', () => {
    const state = buildSitePhaseState({
      site: CAVES_OF_ULUND,
      characters: [ARAGORN],
    });
  ...
  // ─── Movement: Lórien → Caves of Ûlund ─────────────────────────────────────

  test('starter movement from Lórien reaches Caves of Ûlund', () => {
    ...
    const starterEntry = reachable.find(
      r => r.movementType === 'starter' && r.site.id === (CAVES_OF_ULUND as string),
    );

    expect(starterEntry).toBeDefined();
  });

  test('starter movement from Rivendell does NOT reach Caves of Ûlund', () => {
    // Caves of Ûlund's nearestHaven is Lórien, so Rivendell's starter movement
  ...
  // ─── Movement: Caves of Ûlund → Lórien ─────────────────────────────────────

  test('starter movement from Caves of Ûlund reaches Lórien', () => {
    const cavesOfUlund = pool[CAVES_OF_ULUND as string] as SiteCard;
    const allSites = Object.values(pool).filter(isSiteCard);
    const movementMap = buildMovementMap(pool);
  ...
  test('region movement from Lórien reaches Caves of Ûlund (Withered Heath within 4)', () => {
    // Lórien is in Wold & Foothills; Caves of Ûlund is in Withered Heath.
    // Shortest path: Wold & Foothills → Anduin Vales → Grey Mountain Narrows →
    ...
    const regionEntry = reachable.find(
      r => r.movementType === 'region' && r.site.id === (CAVES_OF_ULUND as string),
    );
