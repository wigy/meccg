...
/**
 * Everything a site card carries regardless of alignment.
 *
 * Sites are the destinations for company movement. Each site has a type
 * (determining difficulty and available resources), a path of region types
 ...
 * resources there.
 *
 * Hero, minion, fallen-wizard and balrog sites are structurally identical —
 * they differ only in their `cardType` discriminant (and the hero-only
 * {@link HeroSiteCard.lairOf}), so each alignment's card type extends this
 * base rather than restating the whole shape.
 */
interface SiteCardBase {
   ...
   * Empty for havens, and for Under-deeps sites (which use adjacency instead).
   */
 ...
 *
 * Hero sites are the destinations for hero companies: havens such as
 * Rivendell and Lorien serve as safe bases, everything else is somewhere
 * to play resources — including the Dragon lairs, which are the only sites
 * that carry {@link HeroSiteCard.lairOf}.
 */
export interface HeroSiteCard extends SiteCardBase {
 ...
 */
export interface MinionSiteCard extends SiteCardBase {
  /** Discriminant for the card type union. */
 ...
 */
export interface FallenWizardSiteCard extends SiteCardBase {
  /** Discriminant for the card type union. */
 ...
 */
export interface BalrogSiteCard extends SiteCardBase {
  /** Discriminant for the card type union. */
