...
export interface SetSitePhaseFlagAction extends TriggeredActionBase {
  ...
  readonly flag?: 'hoardBountyAvailable' | 'thoroughSearchAvailable' | 'firstItemNoTapAvailable' | 'hoardKeywordGranted' | 'stolenItemUnlocked';
}
...
export interface GrantCreatureKeyingEffect extends EffectBase {
   ...
   * the count.
   *
   * Used by Umagaur the Pale (dm-112) and Bûthrakaur the Green (dm-105):
   * "Any non-unique Orc or Troll hazard creature can be played (not counting
   * against the hazard limit) on a company that has faced [this creature]
   * that turn."
   */
