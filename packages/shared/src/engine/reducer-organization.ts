...
function handleTransferItem(state: GameState, action: GameAction): ReducerResult {
  ...
  if (itemIndex === -1) return { state, error: `Item ${itemInstId as string} not found on source character` };
...
export function handleStoreItem(state: GameState, action: GameAction): ReducerResult {
  ...
  if (!char) return { state, error: `Character ${charId as string} not found` };
  const itemIndex = char.items.findIndex(i => i.instanceId === itemInstId);
  if (itemIndex === -1) return { state, error: `Item ${itemInstId as string} not found on character` };
...
function handleFetchFromSideboard(state: GameState, action: GameAction): ReducerResult {
  ...
  if (cardIdx === -1) return { state, error: `Sideboard card ${action.sideboardCardInstanceId as string} not found` };
