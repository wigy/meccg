...
export function siteActions(state: GameState, playerId: PlayerId): EvaluatedAction[] {
  ...
  if (siteState.step === 'automatic-attacks'
    ...
    // Repeated/sequenced attacks (Troll-purse re-face, prisoner-rescue) and
    // the `site-entry-roll-attack` gate window: the active player passes to
    // initiate the next attack (or to finish).
