...
function handleRevealOnGuardAttacks(
  ...
  if (action.type === 'reveal-on-guard') {
    ...
    if (ogIdx === -1) return { state, error: `On-guard card ${action.cardInstanceId as string} not found` };
...
function handleInfluenceAttemptDeclare(
  ...
  if (cardIdx === -1) return { state, error: `Faction card ${action.factionInstanceId as string} not found in hand` };
