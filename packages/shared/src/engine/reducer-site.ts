...
function fireCharacterGainsItemChecks(
  ...
  for (const charId of company.characters) {
    ...
    for (const hazard of char.hazards) {
      const hDef = newState.cardPool[hazard.definitionId] as { name?: string; effects?: readonly import('../types/effects.js').CardEffect[] } | undefined;
...
function fireEndOfTurnFetchEffects(state: GameState): GameState {
  ...
  for (const player of newState.players) {
    for (const card of player.cardsInPlay) {
      const def = newState.cardPool[card.definitionId] as { name?: string; effects?: readonly import('../types/effects.js').CardEffect[] } | undefined;
