...
export function reduce(state: GameState, action: GameAction): ReducerResult {
  ...
  if (state.chain != null && (action.type === 'pass-chain-priority' || action.type === 'order-passives' || action.type === 'reveal-on-guard' || action.type === 'cancel-return-to-origin' || action.type === 'counter-cancel-roll')) {
