...
/** True when the definition carries the `environment` keyword. */
function isEnvironmentDefinition(state: GameState, defId: string): boolean {
  const def = state.cardPool[defId as CardDefinitionId];
  return !!def && 'keywords' in def
    && !!(def as { keywords?: readonly string[] }).keywords?.includes('environment');
}

/**
 * Find every environment card currently in play or declared on the active
 * chain, in declaration order (in-play first, then chain entries).
 *
...
export function findEnvironmentTargets(
  ...
  for (const player of state.players) {
    for (const card of player.cardsInPlay) {
      ...
      if (isEnvironmentDefinition(state, card.definitionId as string)) {
  ...
  if (state.chain) {
    for (const entry of state.chain.entries) {
      ...
      if (isEnvironmentDefinition(state, entry.card.definitionId as string)) {
