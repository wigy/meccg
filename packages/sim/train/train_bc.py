...
def main():
    ...
    print(f"model: dims={args.dims} {dims}, {params} parameters")
    ...
    for epoch in range(args.epochs):
        ...
        for start in range(0, len(train), args.batch):
            ...
            glob, ents, emask, cands, cmask, target, _, z = collate(batch, global_width)
            logits, value = net(glob, ents, emask, cands)
    ...
    payload = {
        ...
        "dims": {"preset": args.dims, "values": list(dims)},
        "training": {
            "data": args.data,
