#!/usr/bin/env bash
# Self-play RL loop (P4 v1: REINFORCE with the value head as baseline).
#
# Each iteration: (1) roll out self-play games with the current policy
# *sampling* at temperature 1 (exploration), (2) take one on-policy
# REINFORCE step over those rollouts, (3) gate the updated policy (argmax)
# against the current champion — promotion only when the paired-seed
# Elo-diff 95% lower bound clears GATE_MIN_ELO. Rejected candidates are
# kept on disk but the champion stays.
#
# Usage:
#   train/selfplay_loop.sh <champion-weights.json> <workdir> [iterations]
#
# Env overrides: GAMES (rollout games/iter, default 100), LR (default 1e-4),
# ENTROPY (default 0.01), GATE_PAIRS (default 15), GATE_ROUNDS (default 2),
# GATE_MIN_ELO (default 0), SEED0 (rollout seed base, default 50000).
set -euo pipefail
...
for ((i = 1; i <= ITERS; i++)); do
  ...
  echo "=== iteration $i: REINFORCE step ==="
  python3 train/train_bc.py --mode reinforce --init "$CHAMPION" \
    --data "$rollout" --out "$candidate" --epochs 1 --lr "$LR" --entropy "$ENTROPY" --holdout 0
