...
7. **Iterate until green:** Run these checks **in parallel** and fix any failures. Repeat until all pass:
   - `npm run build` — type-check (must pass)
   - **Changed tests only** — run only the test files added or modified on the PR branch (`git diff --name-only origin/master...HEAD -- '*.test.ts' | xargs npx vitest run`, plus any uncommitted test files); must pass
