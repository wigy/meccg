...
4. **Check each effect against engine support:** For each effect, verify it is actually handled by the game engine. The current implementation status is:
   ...
   **Partially implemented:**
   ...
   - `on-event` — infrastructure exists but `matchesTrigger()` is stubbed to always return false; no triggers fire (`packages/shared/src/engine/chain-reducer.ts`). Exception: `self-enters-play` with `discard-opposing-environments` is implemented directly in the reducer play handlers for environment permanent events (`packages/shared/src/engine/reducer.ts`)
