...
- **Success path:** every edit is committed to a branch (`fix/<slug>`), the branch is pushed, and a PR is open. Tree clean.
- **Failure path:** if you cannot fix the bug — log doesn't corroborate, rules say the behavior is correct, tests won't pass, etc. — you MUST revert all edits before ending the turn: `git checkout -- .` and delete any new files/directories you created. Then emit the result block with `"success": false`.
