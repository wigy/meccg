...
2. Add `'defender-any'` to `assignmentPhase` union type on `CombatState`
   (current: `'cancel-window' | 'defender' | 'attacker' | 'cancel-by-tap' | 'done'`)
3. Add `isCvCC?: boolean` to `CombatState` (discriminator for CvCC-specific logic; absent/false = creature combat)
4. Add `'attacker-character'` to `bodyCheckTarget` union type on `CombatState`
...
3. Add optional `attackingCharacterId?: CardInstanceId` to `AssignStrikeAction` (for CvCC attacker-phase assignment)

### Files: `packages/shared/src/types/state-phases.ts`

4. Add `'declare-company-attack'` to the site phase legal action list (the type union used by `computeLegalActions`)
