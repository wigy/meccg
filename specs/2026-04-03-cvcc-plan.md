...
### Files: `packages/shared/src/types/state-phases.ts`

1. Add `'declare-company-attack'` to `SiteStep` union

### Files: `packages/shared/src/types/state-combat.ts`

1. Add `'defender-any'` to `assignmentPhase` union type on `CombatState`
   (current: `'cancel-window' | 'defender' | 'attacker' | 'cancel-by-tap' | 'done'`)
2. Add `isCvCC?: boolean` to `CombatState` (discriminator for CvCC-specific logic; absent/false = creature combat)
3. Add `'attacker-character'` to `bodyCheckTarget` union type on `CombatState`
   (current: `'character' | 'creature' | null`)
4. Add `attackingCharacterId?: CardInstanceId` to `StrikeAssignment` (which attacking char commits this strike)
5. Add `attackerResult?: 'success' | 'wounded' | 'eliminated'` to `StrikeAssignment`
6. Add `attackerTapToFight?: boolean` to `StrikeAssignment` (tracks attacker's -3 choice)
...
### Files: `packages/shared/src/types/actions-site.ts`
...
2. Export from `actions-site.ts` and add to `GameAction` union in `actions.ts`

### Files: `packages/shared/src/types/actions-movement-hazard.ts`

1. Add optional `attackingCharacterId?: CardInstanceId` to `AssignStrikeAction` (for CvCC attacker-phase assignment)

### Files: `packages/shared/src/types/state-phases.ts` (legal actions)

1. Add `'declare-company-attack'` to the site phase legal action list (the type union used by `computeLegalActions`)

### Initialization

`CombatState` construction sites (creature combat, auto-attack, agent) do not need `isCvCC: false` explicitly since the field is optional (`isCvCC?: boolean`). The `detainment` field should be `false` for CvCC combats.
...
2. **Modify play-resources pass flow**: Currently, pass at `play-resources` calls `advanceSiteToNextCompany()` directly. Change this to transition to `step: 'declare-company-attack'` instead, so the player gets the opportunity to attack before moving on.
...
**`attackerStep1Done`**: The existing `attackerStep1Done` flag (Step 1 — Attacking Player Actions, hazard card window) is not relevant to CvCC since CvCC has no hazard player. During CvCC resolve-strike, skip the attacker-step-1 window and proceed directly to the attacker's -3 sub-step.
...
| File | Changes |
|------|---------|
| `packages/shared/src/types/state-phases.ts` | `SiteStep` extension (`declare-company-attack`) |
| `packages/shared/src/types/state-combat.ts` | `CombatState` (`isCvCC`, `defender-any`, `attacker-character`), `StrikeAssignment` (`attackingCharacterId`, `attackerResult`, `attackerTapToFight`) |
| `packages/shared/src/types/actions-site.ts` | New `DeclareCompanyAttackAction` |
| `packages/shared/src/types/actions-movement-hazard.ts` | `AssignStrikeAction` extension (`attackingCharacterId`) |
| `packages/shared/src/types/actions.ts` | Add `DeclareCompanyAttackAction` to `GameAction` union |
