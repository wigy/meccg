...
- **Defeat cascade** (sweep sister manifestations on defeat, block replays, lair auto-attack suppression): ✓ implemented in step 8. `applyManifestationCascade(state)` runs after every successful action via `postReduce` in `engine/reducer.ts`. The trigger is broad — any chain card landing in `killPile`, `discardPile`, or `outOfPlayPile` causes the cascade. Sister cards from `cardsInPlay` are routed to the **owning** player's `outOfPlayPile` via `ownerOf` (preserves no-card-disappears + the §4.1 MP attribution rule). `isManifestationDefeated` now scans all three off-board piles, so the chain-blocked predicate fires whether the trigger was a defeat (kill pile), removal (out-of-play), or discard. Covered by `rule-metd-dragon-cascade.test.ts`.
