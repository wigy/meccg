...
- Defeating the augmented "at home" Dragon automatic-attack (e.g. Smaug at Home's 2x 18/8 at The Lonely Mountain) never defeated the manifestation itself: `finalizeCombat` only removed a permanent-event's augmented-attack source card for the per-card `permanent-event-auto-attack` `onDefeat: 'remove-from-play'` flag (Balrog of Moria), and the nine Dragon "At Home" permanent-events use a distinct `dragon-at-home` effect, so Smaug at Home stayed in the hazard player's permanent-event box and the lair kept threatening its augmented attack every turn. Per CoE glossary g.man.3 (and 3.IV.1.3, "Defeated Dragon manifestations may be used as trophies") a fully defeated At-Home attack now moves the At-Home card from its owner's `cardsInPlay` to the defender's kill pile, awarding its kill MPs; the existing manifestation cascade then sweeps sister manifestations and the lair's `isManifestationDefeated` check strips its printed Dragon attack for the rest of the game. Covered by a new `rule-metd-dragon-at-home` rules test and a full two-attack regression in the Smaug at Home (td-71) suite. Reported from games mu8n66im-z678rd and mu9mqfu7-a6kpj0 (#3107, #3109)
- Influencing Returned Exiles (td-146) with a character bearing King under the Mountain (td-126) showed the printed +5 bonus when the attempt was declared but resolved with the "other Dwarves" +2: `bearer.itemNames`, which the card's `check-modifier` condition matches against, was only populated in the declare-time `need` computation and not in the pending `faction-influence-roll`'s displayed need or the roll resolution built via `buildBearerContext()`. Both now populate `itemNames`, and a regression test drives the full declare → pending roll → resolution chain asserting the +5 survives at every stage. Reported from game mu8n66im-z678rd (#3108)
- A Chance Meeting (tw-188) never offered home-site-only characters such as Bilbo, Frodo or Sam Gamgee: `recruitViaEventActions` unconditionally enforced the recruit's own `home-site-only` play-flag, which the card's "even a Hobbit" wording exists precisely to override. A new `overridesHomeSiteOnly` field on `recruit-character` (set on A Chance Meeting only; We Have Come to Kill le-252 is unchanged) lifts that flag when the enabling event says so. Reported from game mu8n66im-z678rd, where Bilbo could not be brought in at Ovir Hollow or The Lonely Mountain (#3105)
- A Chance Meeting was also unplayable during the site phase before the company chose to enter its site: the recruit path was only wired into the `play-resources` step, while the other any-phase resource plays were already offered at `select-company` and `enter-or-skip`. Per rule 2.IV.5 a company is at its site for the whole site phase, so the recruit is now offered at those earlier steps too. Reported from game mu8n66im-z678rd (#3106)
- A failed Burglary (td-103) attempt let an ally hosted by the burglar take a strike from the automatic-attack: `soloDefenderInstanceId` excluded every other company member and their allies but carved out an exception for the solo defender's own hosted ally, contradicting the CRF ruling that the character faces the attack "as though he were a one-character company" with "no combat support other than what he himself can provide" (the same wording CRF 22 applies to The Hunt, where a Noble Hound does not shield Alatar). The four failed-burglary sites now set `excludeSoloDefenderAllies` and skip the "each character faces one strike" ally pre-assignment. Reported from game mu8m3270-qg70v3, where Gollum was offered a strike after Gimli's failed burglary (#3104)
- The burglary-attempt roll was invisible to the roller: `applyBurglaryAttemptResolution` rolled via the raw `roll2d6()` helper and stashed the result on `lastDiceRoll` without emitting the `dice-roll` effect that every other 2d6 resolution (flattery, goodwill, riddling, corruption checks, seized-by-terror) produces through `rollDiceForPlayer()`, so the client's dice animation never fired. All three return paths (fizzle outside the site phase, success, failure) now carry the roll effect. Reported from game mu8m3270-qg70v3 (#3103)

### Infrastructure

- Added `bin/top-up-low-credits`, a scriptable equivalent of the admin Users page's "Add X credits" button: it tops up every non-system account whose balance is below its initial credits by 1.5× the consumption since the last credit addition, rounded up to the nearest hundred, with `--dry-run` to preview and `MECCG_LOBBY_URL` / `MECCG_MASTER_KEY` honoured like `bin/requests`. Backed by two new master-key lobby routes, `GET /api/system/players` and `POST /api/system/players/:name/credits/top-up`, which share the top-up helper with the admin route so both add the identical amount; accounts with nothing consumed since their last addition are reported as skipped (#3110)
...
- Certified Palm to Palm (dm-153): a Haven-only, Wizard-less company condition via a new `target.hasWizard` company play-target filter field (backed by a `companyHasWizard` helper mirroring `companyHasRingwraith`/`companyHasBalrog`); +1 mind to every character and every ally in the company (`company-modifier` now accepts the `mind` stat and a new `appliesTo: "allies"` field folded into `allyEffectiveMind`); and a new `grant-attempt-support` DSL primitive letting company members tap in support of another member's influence attempt or corruption-removal roll for +1 — a capability CoE only grants natively for combat and end-of-game corruption checks — wired into both the faction-influence-attempt and `remove-self-on-roll` emitters via a shared `supportCharacterId` field. `company-membership-changes` gained optional `when` support ("a Wizard joins") and a new `character-splits-off-company` on-event covers CoE 2.II.3.6, fired from `split-company` and `move-to-company`. Also fixes a latent bug where the opponent-influence-attempt emitter read an ally's mind straight off its definition, ignoring `statOverride` (Ready to His Will le-220) and company-bound modifiers (#3100)
- Certified Forod (td-117): "moving without region cards" between Lindon, Forochel, Angmar, Gundabad, Grey Mountain Narrows, Withered Heath and the Iron Hills with a triple-wilderness site path and a hazard limit of -2 (floor 2) — the same mechanic as Belegaer (td-100) keyed to a different region list. Rather than a third hardcoded constant, `set-company-special-movement` gained a data-driven `named-region-crossing` mode whose region list, site-path region types and hazard-limit delta/floor ride on the granting card's effect (`crossingRegions`/`crossingSitePath`/`crossingHazardLimitDelta`/`crossingHazardLimitFloor`), so the sibling cards Harad (td-121) and Rhûn (td-147) can reuse it; threaded through the movement planner, site-path resolution, hazard-limit snapshot, declare-path and end-of-turn clearing. Belegaer keeps its existing path untouched (#3099)
- Certified Wielded Twice (td-167): a new `item-untap` DSL primitive (documented in `docs/card-effects-dsl.md` §43f) — the company-scoped sibling of `site-untap` — lets a tapping sage untap one tapped item borne by any character in his own company, with an ally sage mapped back to its host via `findAttachment`. Wired into both generic short-event dispatchers, the chain of effects and resolution via `applyShortEventItemUntap`; the card's missing `ritual` keyword was restored from the authoritative database (#3095)
- Unabated in Malice (ba-26) could only be placed on-guard, costing a hazard-limit slot, because a blanket suppression of open M/H play for from-hand modify-attack cards did not distinguish a card gated on `attack.automatic: true` — which CoE 9.3.2.3 lets target an automatic-attack "at any time while its site is in play" — from one gated on a live-attack property like `enemy.race` (Black Vapour ba-14). When the `when` condition matches a synthetic automatic-attack context and the destination site has an automatic-attack, the card is now playable openly in M/H and installs a single-use `pending-attack-modifier` constraint (mirroring Arouse Defenders' `auto-attack-boost`) that the site phase's automatic-attack initiation consumes, applying the strike/prowess/body deltas and arming cancel protection (#3098)

### Web Client

- Clicking a character during the automatic-attacks step whose only legal action was `declare-burglary` dispatched it immediately, so a player who meant to face the automatic-attack normally silently committed Pallando to a burglary attempt. Since Burglary is now directly clickable from the hand, the character-click path always shows the confirmation menu, even with a single eligible card/character combination. Reported from game mu7ik4td-ewl6us (#3097)
- Once Burglary (td-103) was declared, neither the Roll button nor the "Waiting..." indicator appeared for the `burglary-attempt` pending resolution — the pass-like whitelist in `button-classification.ts` omitted it, so the game read as frozen. It is now whitelisted with a "Roll" label, the same fix previously applied to `resolve-dice-check`, `flattery-attempt`, `seized-by-terror-roll` and `gold-ring-test-roll`. Reported from game mu77cexd-dexv27 (#3096)
...
- Test of Form (tw-338/tw-339), Test of Lore (tw-340) and Test of Fire (le-239) carried a spurious `play-window: { phase: "organization" }` effect not supported by their card text or CoE 6.2, so they could not be played outside the organization phase; per CoE 2.1.1 a resource short-event with no play-window restriction is playable during any phase of the resource player's own turn, as the sibling Wizard's Test already was. Reported from game mu4kj7vp-pj1r4b where Test of Form was rejected during the site phase (#3079)
- Wizard's Test (tw-365) played outside the organization or site phase went through the generic hero short-event path, which offered a bare character target without crossing the Wizard with the gold rings in his company; the `enqueue-gold-ring-test` apply then silently fizzled and only the paired corruption check fired. The movement/hazard-phase path now emits the same `targetGoldRingInstanceId` crossing as the organization-phase path, also covering Test of Fire (#3078)
- CoE 3.47's end-of-organization-phase influence overflow discard kept re-offering a character protected by Elf-song (tw-223) as its only candidate: the discard fizzled, the overflow remained, and the game froze in an infinite loop. `removableCharacters` now excludes removal-protected characters, and when every candidate is protected the overflow goes unresolved instead of deadlocking, matching the existing all-avatar case. Reported from game mu4k55yg-f355xa where Elf-song kept in play via Echo of All Joy protected every character at Rivendell (#3077)
- Item salvage after an eliminated character (CoE 3.I.2) offered every item to every unwounded companion regardless of the item's own play restrictions, so a Wizard's Ring (tw-363, "Wizard only", cannot be transferred per CRF 22) could be handed to a non-Wizard. `itemSalvageActions` now skips no-transfer items entirely and any item/recipient pairing that fails the item's character filter; an item with no eligible recipient is discarded. Reported from game mu4dwlj6-tgp0q2 (#3076)
- Geann a-Lisch's (le-374) hazard-keying override, which reinterprets a company's site path as a virtual five-region path "for purposes of playing and interpreting hazards", leaked into the end-of-M/H "for each region moved through" corruption checks, so a stationary company at the site was charged five Alone and Unadvised checks (and four Lure of Nature checks) for a path it never travelled. `MovementHazardPhaseState.traveledSitePath` now records the real path before the override is applied and the corruption-check sweep reads that instead. Reported from games mu436f0m-e60pkn and a second game where Ôm-buri-Ôm was charged nine checks (#3075)
- Morgul-orcs (as-119) had an empty `playableAt` and influence number 0, so the faction could never be played at Minas Morgul; the definition now matches its printed text (playable at Minas Morgul, influence number 9, in-play influence 0, Standard Modifications Ungol-orcs -2 / Orcs of Angmar +2) (#3074)
- A second copy of Open to the Summons (wh-46) offered in lieu of a minor item during the item draft could be placed on an agent that already carried one auto-paired at draft finalize, violating "cannot be duplicated on a given character"; both the item-draft eligibility filter and the place-starting-company-event handler now honour the card's character-scoped duplication limit. Reported from game mu436f0m-e60pkn where two copies landed on Wormtongue (#3073)
- Certified Which Might Be Lies (dm-100): a hazard short-event that discards an opponent's stored resource permanent-event which required a site where a given resource (Information) is playable. New `stored-permanent-event` play target with a `requiresResource` filter scanning the marshalling-point pile, a `discard-stored-permanent-event` resolution effect routing the card to its owner's discard pile, and the "requires a site where X is playable" recogniser shared with the `permanent-event-mp` override (#3067)
- Certified Palantír of Annúminas (le-331): tap to fetch a sage-only card from the play deck or discard pile, mirroring the identical hero printing (tw-297) via the existing enqueue-pending-fetch mechanism gated on the bearer being able to use a Palantír (#3080)
- Certified Belegennon (td-185): hoard-gated item play, unconditional +1 body (max 9), the Reforging family's restore-item grant (4 MP / 3 CP when restored) shared with Horn of Defiance and Ringil, and a Warrior-and-restored-gated untap-penalty cancellation; the missing `armor` keyword was added to match the printed text (#3068)
- Certified Ôm-buri-Ôm (dm-21): +3 direct influence against Wose factions, the same pattern as Pôn-ora-Pôn (#3072)
- Certified Morgul-orcs (as-119), Hillmen (le-269) and Dunlendings (le-263): the minion factions' Standard Modification check-modifier effects (Hillmen: Dunlendings +2, Angmarim +2; Dunlendings: Hillmen +2) gated on the controller having the named faction in play, following the Easterlings precedent, with tests covering each modification individually and stacked, per-player scoping and the site restrictions (#3071, #3070, #3069)
