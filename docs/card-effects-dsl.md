...
The same context also carries `bearer` (race / skills / name / base stats), and
the effect may sit on an **item** rather than the character, so an item can
raise its holder's hand size only while a qualifying character bears it — e.g.
Palantír of Elostirion (le-332), "If the bearer is a sage: your hand size
increases by one":
