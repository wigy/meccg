...
- `scope` — zone to enumerate relative to the bearer. Supported:
  ...
  - `"bearer-items"` — items borne by the bearer character himself only
    (never a company-mate's item, unlike `"company-items"`). Used by Magic
    Ring of Lore (tw-272): "he may tap to use a Palantír" — "he" is the
    ring's own bearer, so only a Palantír *he* bears qualifies.
