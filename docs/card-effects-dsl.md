...
### 79. `cvcc-capture-in-lieu-of-body-check` + `eliminate-captured-character` (No Better Use)
...
- **The capture itself** (`captureCharacterInLieuOfBodyCheck`) reuses the
  ...
  the bare card is marked with a `character-pressed` active constraint
  (negative MP, 0 GI, never untaps/heals — the same constraint kind
  Press-gang uses, so `recompute-derived.ts` / `reducer-untap.ts` /
  `influence-overflow.ts` already score and lock it correctly with no new
...
`character-pressed` constraint — already stripped of every possession at
