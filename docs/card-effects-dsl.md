...
- `untap-phase-end` -- like `untap-phase-at-haven` but fires regardless of the bearer's site. Scans every character of the active player (no haven check) for attached cards carrying this on-event and enqueues a `corruption-check` pending resolution per match. Used by *The Least of Gold Rings*.
