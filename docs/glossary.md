...
- **Pending Resolution** — A discrete piece of work the engine has queued for a specific player to resolve before continuing. Stored at the top of `GameState` as `pendingResolutions: PendingResolution[]`. While any entry targets the actor of an incoming action, the legal-action menu collapses to "resolve the top entry" — replacing the per-phase ad-hoc fields the engine used to carry (`pendingCorruptionCheck`, `pendingWoundCorruptionChecks`, etc.). Drains FIFO per actor and is auto-swept at the matching scope boundary. Owned by `engine/pending.ts`; resolved by handlers in `engine/pending-reducers.ts`. See `specs/2026-04-08-pending-effects-plan.md`.
- **Active Constraint** — A scoped restriction on the legal actions available to some target (company, character, or player). Stored at the top of `GameState` as `activeConstraints: ActiveConstraint[]`. Filters the legal-action menu while it lives; auto-clears at the matching scope boundary. Used by cards like *Lost in Free-domains*, *River*, and *Stealth* that modify what a player can do for some scope without queuing work to resolve. Owned by `engine/pending.ts`; applied by `legal-actions/pending.ts`.
...
- **Site-Bound Badge** — A text label shown on a permanent event/resource card in the flat `cards-in-play-row` when it is bound to a site (`attachedToSite`) that is not currently occupied by any company, naming the site it belongs to. Cards bound to a currently-occupied site instead render beneath that site directly (see the site-attachments strip).
...
- **Text Log** — The UI area where game actions and notifications are printed as text, giving players a running transcript of what has happened in the game.
...
- **Score Box** — The `.deck-box-score` element (`#self-score` / `#opponent-score`) inside each deck box. Displays two metric chips side by side: **MP** (tournament marshalling point total) and **GI** (remaining general influence). Hovering either chip shows a tooltip — MP shows a per-category breakdown table for both players; GI lists the characters currently under general influence with their mind values. Populated by `renderPlayerNames()` in `render-player-names.ts`.
- **Radar** — A compact fixed-position minimap widget (`.map-radar`) shown in the bottom-right corner during gameplay (single-company view, all-company view, and battle screen). Displays the Middle-Earth map with coloured dots for all companies: the currently viewed own company pulses green, other own companies are solid green, the currently viewed opponent company pulses red, other opponent companies are solid red, and destination sites appear in light green (own) or light red (opponent). Own agents are shown as small diamond markers. Clicking the radar opens the full-screen map overlay. When the active company is at an Under-deeps site, the radar switches to the Under-deeps schematic with a ⛏ indicator. Implemented in `map-radar.ts`; styles in `style-map.css`.
- **Full-Screen Map** — A modal overlay (`.map-fullscreen-overlay`) opened by clicking the radar. Shows the full Middle-Earth map with larger dots and tooltips displaying company name and current site. Own company dots are clickable to switch the viewed company. Supports toggling between the surface map and the Under-deeps schematic. Implemented in `map-fullscreen.ts`.
