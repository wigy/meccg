...
- **Pending Resolution** — A discrete piece of work the engine has queued for a specific player to resolve before continuing. Stored at the top of `GameState` as `pendingResolutions: PendingResolution[]`. While any entry targets the actor of an incoming action, the legal-action menu collapses to "resolve the top entry" — replacing the per-phase ad-hoc fields the engine used to carry (`pendingCorruptionCheck`, `pendingWoundCorruptionChecks`, etc.). Drains FIFO per actor and is auto-swept at the matching scope boundary. Owned by `engine/pending.ts`; resolved by handlers in `engine/pending-reducers.ts`. See `specs/pending-effects-plan.md`.
